...
local Library do
	...
	local Themes = { -- edit this to change the theme
		...
		["Dracula"] = {
			...
			["Inline"] = FromRGB(48, 50, 65),
			["Border"] = FromRGB(68, 71, 90),
			["Shadow"] = FromRGB(0, 0, 0),
			["Text"] = FromRGB(248, 248, 242),
			["Inactive Text"] = FromRGB(150, 150, 160),
			["Accent"] = FromRGB(189, 147, 249),
			["Element"] = FromRGB(55, 57, 73),
			["Gradient"] = FromRGB(180, 180, 190)
		},

		["Neon"] = {
			["Background"] = FromRGB(8, 8, 12),
			["Inline"] = FromRGB(14, 14, 20),
			["Border"] = FromRGB(0, 220, 180),
			["Shadow"] = FromRGB(0, 200, 160),
			["Text"] = FromRGB(230, 255, 250),
			["Inactive Text"] = FromRGB(120, 180, 160),
			["Accent"] = FromRGB(0, 255, 200),
			["Element"] = FromRGB(18, 30, 28),
			["Gradient"] = FromRGB(0, 200, 160)
		},

		["Midnight"] = {
			["Background"] = FromRGB(10, 10, 18),
			["Inline"] = FromRGB(16, 16, 28),
			["Border"] = FromRGB(30, 30, 55),
			["Shadow"] = FromRGB(0, 0, 20),
			["Text"] = FromRGB(220, 220, 255),
			["Inactive Text"] = FromRGB(130, 130, 170),
			["Accent"] = FromRGB(120, 100, 255),
			["Element"] = FromRGB(22, 22, 40),
			["Gradient"] = FromRGB(160, 150, 220)
		}
	}
	...
	local Tween = { } do
		...
		Tween.Clean = function(self)
			...
			self:Pause()          -- was incorrectly calling Tween:Pause() (class) instead of self:Pause()
			self.Tween:Destroy()
			self.Tween = nil
		end
	end
	...
	-- Returns a shallow copy of all current Flags
	Library.GetAllFlags = function(self)
		return TableClone(self.Flags)
	end

	-- Returns true if FlagName exists and is not nil
	Library.HasFlag = function(self, FlagName)
		return self.Flags[FlagName] ~= nil
	end

	-- Resets a single flag (or ALL flags if no name given) to nil by re-invoking
	-- each registered SetFlag with its current default (false / 0 / nil)
	Library.ResetFlags = function(self, FlagName)
		if FlagName then
			local SetFn = self.SetFlags[FlagName]
			if SetFn then SetFn(nil) end
		else
			for Name, SetFn in self.SetFlags do
				Library:SafeCall(SetFn, nil)
			end
		end
	end
	...
	-- Returns the current value of a flag, or nil if it doesn't exist
	Library.GetFlag = function(self, FlagName)
		return self.Flags[FlagName]
	end

	-- Sets a flag value and fires its registered callback if one exists
	Library.SetFlag = function(self, FlagName, Value)
	...
	-- Typed notification shorthand: type is "success" | "error" | "info" | "warning"
	Library.Toast = function(self, Title, Description, NotifType, Duration)
		NotifType = NotifType or "info"
		Duration  = Duration or 3

		local TypeColors = {
			["success"] = FromRGB(80, 220, 130),
			["error"]   = FromRGB(240, 80,  80),
			["warning"] = FromRGB(240, 190, 60),
		...
		local Prefix      = TypeIcons[NotifType]  or ""
		...
		local Items = { } do
			...
			-- Left accent stripe for type colour
			Items["Stripe"] = Instances:Create("Frame", {
				Parent = Items["Notification"].Instance,
				BackgroundColor3 = AccentColor,
				BorderSizePixel = 0,
				Size = UDim2New(0, 3, 1, -16),
				Position = UDim2New(0, 0, 0, 8)
			})

			Instances:Create("UICorner", {
				Parent = Items["Stripe"].Instance,
	...
	Library.Version = "1.1.0-modded"

	-- Fires Callback once when Library:Unload() is called
	Library.OnUnload = function(self, Callback)
		local OriginalUnload = self.Unload
		self.Unload = function(s)
			Library:SafeCall(Callback)
			OriginalUnload(s)
		end
	end

	-- Toggle the main window visibility with an optional tween
	Library.ToggleWindow = function(self, Window)
		if not Window or not Window.Items or not Window.Items["MainFrame"] then return end
		local Frame = Window.Items["MainFrame"].Instance
		Window.IsOpen = not Window.IsOpen
		Frame.Visible = Window.IsOpen
	end

	-- Show a blocking confirmation dialog; calls OnConfirm or OnCancel
	Library.Confirm = function(self, Title, Message, OnConfirm, OnCancel)
		...
		local Dialog = Instances:Create("Frame", {
			...
			Size = UDim2New(0, 300, 0, 130),
			ZIndex = 201
		}) Dialog:AddToTheme({BackgroundColor3 = "Background"})
		...
		local function MakeBtn(Text, XPos, Accent)
			local Btn = Instances:Create("TextButton", {
				...
				AutoButtonColor = false,
				BorderSizePixel = 0,
				TextColor3 = Accent and Library.Theme.Background or Library.Theme.Text,
				TextSize = 13,
				BackgroundColor3 = Accent and Library.Theme.Accent or Library.Theme.Element,
				Size = UDim2New(0, 125, 0, 28),
				Position = UDim2New(0, XPos, 1, -38),
				ZIndex = 202
			})

			if Accent then
				Btn:AddToTheme({BackgroundColor3 = "Accent", TextColor3 = "Background"})
			else
				Btn:AddToTheme({BackgroundColor3 = "Element", TextColor3 = "Text"})
			end

			Instances:Create("UICorner", { Parent = Btn.Instance, CornerRadius = UDimNew(0, 6) })
			return Btn
		end

		local ConfirmBtn = MakeBtn("Confirm", 10, true)
		local CancelBtn  = MakeBtn("Cancel", 145, false)
		...
		ConfirmBtn:Connect("MouseButton1Down", function()
			CloseDialog()
			if OnConfirm then Library:SafeCall(OnConfirm) end
		end)

		CancelBtn:Connect("MouseButton1Down", function()
			CloseDialog()
			if OnCancel then Library:SafeCall(OnCancel) end
		end)
	end
	...
	local Components = { } do
		...
		Components.Progress = function(Data)
			...
			local Items = { } do
				...
				Items["ValueLabel"] = Instances:Create("TextLabel", {
					...
					BackgroundColor3 = FromRGB(255, 255, 255)
				}) Items["ValueLabel"]:AddToTheme({TextColor3 = "Inactive Text"})
			...
			function Progress:Set(Value)
				...
				local Pct = MathFloor(Value * 100)
				Items["ValueLabel"].Instance.Text = Pct .. "%"
				Items["Fill"]:Tween(nil, {Size = UDim2New(Value, 0, 1, 0)})
			...
			function Progress:SetDisabled(Bool)
				self.Disabled = Bool
				local t = Bool and 0.6 or 0
				Items["Text"]:Tween(nil, {TextTransparency = t})
				Items["Track"]:Tween(nil, {BackgroundTransparency = t})
				Items["Fill"]:Tween(nil, {BackgroundTransparency = t})
			end

			local SearchData = {Name = Data.Name, Item = Items["Progress"]}
			local PageSearchData = Library.SearchItems[Data.Page]
			if PageSearchData then
				TableInsert(PageSearchData, SearchData)
			end

			Progress:Set(Data.Default or 0)

			Library.SetFlags[Progress.Flag] = function(Value)
				Progress:Set(Value)
			end
		...
		Components.Separator = function(Data)
			...
			local Items = { } do
				Items["Separator"] = Instances:Create("Frame", {
					...
					Name = "\0",
					BackgroundTransparency = 1,
					Size = UDim2New(1, 0, 0, Data.Name and 18 or 8),
					BorderSizePixel = 0,
					ZIndex = 2,
					BackgroundColor3 = FromRGB(255, 255, 255)
				})
		...
		-- ── Radio Group ───────────────────────────────────────────────────────
		-- Single-select group of labelled radio buttons.
		-- Data.Items = {"Option1", "Option2", ...}
		-- Data.Default = "Option1"   (optional)
		Components.RadioGroup = function(Data)
			local RadioGroup = {
				Value    = nil,
				Flag     = Data.Flag,
				Disabled = false,
			...
			Library.Flags[RadioGroup.Flag] = nil
			...
			local Items = { } do
				...
				Items["OptionsFrame"] = Instances:Create("Frame", {
					Parent = Items["Container"].Instance,
					Name = "\0",
					BackgroundTransparency = 1,
			...
			local function SelectOption(OptionName)
				if RadioGroup.Disabled then return end

				RadioGroup.Value = OptionName
				Library.Flags[RadioGroup.Flag] = OptionName

				for Name, Btn in RadioGroup.Buttons do
					local Active = (Name == OptionName)
					Btn.Dot:Tween(nil, {BackgroundTransparency = Active and 0 or 1})
					Btn.DotOuter:Tween(nil, {BackgroundColor3 = Active and Library.Theme.Accent or Library.Theme.Element})
					Btn.Text:Tween(nil, {TextTransparency = Active and 0 or 0.4})
				end

				if Data.Callback then Library:SafeCall(Data.Callback, OptionName) end
				if RadioGroup.OnChanged then RadioGroup.OnChanged(OptionName) end
			end

			function RadioGroup:Add(OptionName)
				...
				local DotOuter = Instances:Create("Frame", {
					Parent = Row.Instance,
					Name = "\0",
					AnchorPoint = Vector2New(1, 0.5),
					Position = UDim2New(1, 0, 0.5, 0),
				...
				}) DotOuter:AddToTheme({BackgroundColor3 = "Element"})

				Instances:Create("UICorner", {
					Parent = DotOuter.Instance,
					CornerRadius = UDimNew(1, 0)
				})
				...
				RadioGroup.Buttons[OptionName] = BtnData
			...
			function RadioGroup:Set(OptionName)
				SelectOption(OptionName)
			end

			function RadioGroup:SetDisabled(Bool)
				self.Disabled = Bool
				local t = Bool and 0.6 or 0
				Items["Label"]:Tween(nil, {TextTransparency = t})
				for _, Btn in self.Buttons do
					Btn.Text:Tween(nil, {TextTransparency = Bool and 0.6 or (self.Value == _ and 0 or 0.4)})
					Btn.DotOuter:Tween(nil, {BackgroundTransparency = t})
				end
			end

			function RadioGroup:SetVisible(Bool)
				Items["Container"].Instance.Visible = Bool
			end

			local SearchData = { Name = Data.Name, Item = Items["Container"] }
			local PageSearchData = Library.SearchItems[Data.Page]
			if PageSearchData then
				TableInsert(PageSearchData, SearchData)
			end

			for _, OptionName in Data.Items or { } do
				RadioGroup:Add(OptionName)
			end

			if Data.Default then
				RadioGroup:Set(Data.Default)
			end

			Library.SetFlags[RadioGroup.Flag] = function(Value)
				RadioGroup:Set(Value)
			end
			...
			function Slider:SetMin(Value)
				Data.Min = Value
				-- Re-apply current value so fill bar re-clamps correctly
				Slider:Set(MathClamp(self.Value, Data.Min, Data.Max))
			end
	...
	Library.Notification = function(self, Text, Description, Duration)
		...
		task.delay(Duration, function()
			Tween:Create(Items["Notification"].Instance, TweenInfo.new(0.3), {BackgroundTransparency = 1}, true)
			Tween:Create(Items["Title"].Instance,        TweenInfo.new(0.3), {TextTransparency = 1},       true)
			Tween:Create(Items["Description"].Instance,  TweenInfo.new(0.3), {TextTransparency = 1},       true)
			task.wait(0.3)
			Items["Notification"]:Clean()
		end)
	end
	...
	Library.Window = function(self, Properties)
		...
		-- Fixed: was referencing Items["BackgroundImage"] which doesn't exist
		function Window:SetBackgroundImage(Value)
			Items["ImageBackground"].Instance.Image = tostring(Value)
		end

		local RenderStepped

		Items["Input"]:Connect("FocusLost", function()
			-- Stop the per-frame search scan and restore all items
			if RenderStepped then
				RenderStepped:Disconnect()
				RenderStepped = nil
			end
			...
			if PageSearchData then
				for _, Value in PageSearchData do
					Value.Item.Instance.Visible = true
				end
			end
		...
		Items["Input"]:Connect("Focused", function()
			...
			RenderStepped = RunService.RenderStepped:Connect(function()
				...
				for Index, Value in PageSearchData do 
					...
					elseif StringFind(StringLower(Name), Query, 1, true) then
						Element.Instance.Visible = true
					else
		...
		-- NEW: Change the window title text at runtime
		function Window:SetTitle(NewTitle)
			self.Name = tostring(NewTitle)
			Items["Title"].Instance.Text = tostring(NewTitle)
		end

		-- NEW: Change the background image icon at runtime
		function Window:SetIcon(AssetId)
			Items["ImageBackground"].Instance.Image = tostring(AssetId)
		end

		-- NEW: Resize the window frame with a tween
		function Window:SetSize(NewSize)
			Items["MainFrame"]:Tween(nil, {Size = NewSize})
		end
	...
	Library.Sections.Label = function(self, Text, Alignment, Tooltip, Outline)
		...
		function Label:SetDisabled(Bool)
	...
	Library.Sections.Progress = function(self, Properties)
		...
		function Progress:OnChanged(Callback)
			NewProgress.OnChanged = Callback
		end
	...
	-- ── Section:Separator ────────────────────────────────────────────────────
	Library.Sections.Separator = function(self, Name)
		local Sep = {
			Window  = self.Window,
			Page    = self.Page,
			Section = self
		...
		local NewSep, SepItems = Components.Separator({
			Name   = Name or "",
			Parent = self.Items["Content"]
		})

		function Sep:SetVisible(Bool)
			NewSep:SetVisible(Bool)
		end
	...
	-- ── Section:RadioGroup ───────────────────────────────────────────────────
	Library.Sections.RadioGroup = function(self, Properties)
		Properties = Properties or { }
		...
		local RadioGroup = {
			...
			Name     = Properties.Name     or Properties.name     or "RadioGroup",
			Flag     = Properties.Flag     or Properties.flag     or Library:NextFlag(),
			Default  = Properties.Default  or Properties.default  or nil,
			Items    = Properties.Items    or Properties.items    or { },
			Callback = Properties.Callback or Properties.callback or function() end,
		...
		local NewRadioGroup, RadioGroupItems = Components.RadioGroup({
			Name     = RadioGroup.Name,
			Flag     = RadioGroup.Flag,
			Default  = RadioGroup.Default,
			Items    = RadioGroup.Items,
			Parent   = RadioGroup.Section.Items["Content"],
			Callback = RadioGroup.Callback,
			Page     = RadioGroup.Page,
			OnChanged = RadioGroup.OnChanged,
			Disabled = RadioGroup.Disabled
		})

		RadioGroupItems["Container"]:Tooltip(RadioGroup.Tooltip)

		function RadioGroup:Set(Value)
			NewRadioGroup:Set(Value)
		end

		function RadioGroup:Add(OptionName)
			return NewRadioGroup:Add(OptionName)
		end

		function RadioGroup:SetDisabled(Bool)
			NewRadioGroup:SetDisabled(Bool)
		end

		function RadioGroup:SetVisible(Bool)
			NewRadioGroup:SetVisible(Bool)
		end

		function RadioGroup:OnChanged(Callback)
			NewRadioGroup.OnChanged = Callback
		end

		return RadioGroup
	end

	-- ── Section:Colorpicker (standalone, not sub-element) ────────────────────
	Library.Sections.Colorpicker = function(self, Properties)
		Properties = Properties or { }
		...
		-- Use the Label helper as a host row, attach sub colorpicker to it
		local LabelRow = self:Label(Colorpicker.Name, "Left", Colorpicker.Tooltip)

		local NewColorpicker = LabelRow:Colorpicker({
			Name     = Colorpicker.Name,
			Flag     = Colorpicker.Flag,
			Default  = Colorpicker.Default,
			Alpha    = Colorpicker.Alpha,
		...
		function Colorpicker:SetDisabled(Bool)
			NewColorpicker:SetDisabled(Bool)
		end

		function Colorpicker:SetVisible(Bool)
			NewColorpicker:SetVisible(Bool)
		end

		function Colorpicker:OnChanged(Callback)
			NewColorpicker.OnChanged = Callback
		end
	...
	Library.CheckForAutoLoad = function(self)
		...
		if Success then 
			Library:Notification("Success!", "Successfully autoloaded config.", 5)
					...
					do
						...
						LoadSaveButton:Add("Load", function()
							if ConfigSelected then
								...
								if Success then 
									Library:Notification("Success!", "Successfully loaded config.", 5)
						...
						LoadSaveButton:Add("Save", function()
							if ConfigSelected then
								...
								if not Success then 
									...
									Library:Notification("Success!", "Successfully saved config.", 5)
								end
							end
						end, false)
						...
						AutoloadButton:Add("Set autoload", function()
							if ConfigSelected then 
								...
								Library:Notification("Success!", "Successfully set autoload.", 5)
							end
						end)
					...
					do
						...
						LoadSaveButton:Add("Load", function()
							if ThemeSelected then
								...
								if Success then 
									Library:Notification("Success!", "Successfully loaded theme.", 5)
						...
						LoadSaveButton:Add("Save", function()
							if ThemeSelected then  
								...
								if not Success then 
									...
									Library:Notification("Success!", "Successfully saved theme.", 5)
								end
							end
						end, false)
