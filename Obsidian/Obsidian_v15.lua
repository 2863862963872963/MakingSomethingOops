...
local Library = {
    ...
    Scheme = {
        ...
        AccentColor = Color3.fromRGB(110, 75, 255),
        OutlineColor = Color3.fromRGB(38, 38, 46),
        FontColor = Color3.fromRGB(230, 230, 240),
        Font = Font.fromEnum(Enum.Font.Code),

        RedColor = Color3.fromRGB(230, 55, 75),
    ...
    Themes = {
        ...
        Midnight = {
            BackgroundColor = Color3.fromRGB(8, 10, 20),
            MainColor      = Color3.fromRGB(14, 17, 32),
            AccentColor    = Color3.fromRGB(70, 130, 255),
            OutlineColor   = Color3.fromRGB(30, 36, 60),
            FontColor      = Color3.fromRGB(210, 220, 255),
        },
        Rose = {
            BackgroundColor = Color3.fromRGB(14, 10, 12),
            MainColor      = Color3.fromRGB(24, 16, 20),
            AccentColor    = Color3.fromRGB(240, 80, 120),
            OutlineColor   = Color3.fromRGB(48, 30, 38),
            FontColor      = Color3.fromRGB(255, 220, 230),
        },
        Emerald = {
            BackgroundColor = Color3.fromRGB(8, 14, 10),
            MainColor      = Color3.fromRGB(12, 22, 16),
            AccentColor    = Color3.fromRGB(50, 200, 110),
            OutlineColor   = Color3.fromRGB(24, 44, 30),
            FontColor      = Color3.fromRGB(200, 255, 220),
        },
        Crimson = {
            BackgroundColor = Color3.fromRGB(14, 8, 8),
            MainColor      = Color3.fromRGB(24, 12, 12),
            AccentColor    = Color3.fromRGB(220, 50, 50),
            OutlineColor   = Color3.fromRGB(50, 22, 22),
            FontColor      = Color3.fromRGB(255, 210, 210),
        },
        Light = {
            BackgroundColor = Color3.fromRGB(240, 240, 245),
            MainColor      = Color3.fromRGB(255, 255, 255),
            AccentColor    = Color3.fromRGB(100, 70, 240),
            OutlineColor   = Color3.fromRGB(200, 200, 210),
            FontColor      = Color3.fromRGB(20, 20, 30),
        },
    },
...
-- Applies a named theme preset or a custom Scheme table.
-- Usage: Library:SetTheme("Midnight")  or  Library:SetTheme({ AccentColor = Color3.fromRGB(255,100,0) })
function Library:SetTheme(ThemeOrName)
    local Preset
    if typeof(ThemeOrName) == "string" then
        Preset = Library.Themes[ThemeOrName]
        assert(Preset, string.format("Unknown theme %q. Available: %s", ThemeOrName, table.concat(Library:GetThemeNames(), ", ")))
    elseif typeof(ThemeOrName) == "table" then
        Preset = ThemeOrName
    else
        error("SetTheme expects a string name or a table of Scheme overrides.")
    end

    for Key, Value in Preset do
        if Library.Scheme[Key] ~= nil or Key == "BackgroundColor" or Key == "MainColor" or Key == "AccentColor" or Key == "OutlineColor" or Key == "FontColor" then
            Library.Scheme[Key] = Value
        end
    end

    -- Light theme detection based on FontColor brightness
    if Preset.FontColor then
        local R, G, B = Preset.FontColor.R, Preset.FontColor.G, Preset.FontColor.B
        Library.IsLightTheme = (R * 0.299 + G * 0.587 + B * 0.114) < 0.5
    end

    Library:UpdateColorsUsingRegistry()
end

-- Returns all registered theme names as a sorted list.
...
-- Registers a new custom theme preset.
function Library:AddTheme(Name: string, Preset: { [string]: Color3 })
    assert(typeof(Name) == "string", "Theme name must be a string.")
    assert(typeof(Preset) == "table", "Theme preset must be a table.")
    Library.Themes[Name] = Preset
end

-- Transparency roles: each scheme key → a multiplier (0–1) applied to the user's Alpha.
-- Different layers fade at different rates so the UI looks graduated, not flat.
local TransparencyRoles = {
    BackgroundColor = 1.00,  -- outermost shell — full alpha
    MainColor       = 0.85,  -- panels / groupboxes
    AccentColor     = 0.50,  -- accent elements stay more visible
    OutlineColor    = 0.00,  -- borders never go transparent
}

-- Sets overall UI transparency (0 = opaque, 1 = fully transparent).
-- Applies graduated levels per layer so the UI looks uniform, not flat.
-- Also correctly fades the active tab-highlight background.
function Library:SetUITransparency(Alpha: number)
    Alpha = math.clamp(Alpha, 0, 1)
    Library.UITransparency = Alpha

    for Instance, Props in Library.Registry do
        if not Props.BackgroundColor3 then continue end

        local Role = typeof(Props.BackgroundColor3) == "string" and Props.BackgroundColor3 or nil
        local Multiplier = (Role and TransparencyRoles[Role]) or 0.85

        pcall(function()
            Instance.BackgroundTransparency = Alpha * Multiplier
        end)
    end
end
...
--[[
    Library:CustomToggleUi(Info)

    Info fields:
        Shape         = "Circle" | "Square" | "Rounded"   (default "Rounded")
        Size          = UDim2                              (default UDim2.fromOffset(40, 40))
        Position      = UDim2                              (default top-left corner)
        OpenedIcon    = string  lucide/sf/solar/rbxasset   (icon when UI is open)
        ClosedIcon    = string                             (icon when UI is closed)
        OpenedColor   = Color3                             (bg tint when open,  default AccentColor)
        ClosedColor   = Color3                             (bg tint when closed, default MainColor)
        OnlyMobile    = bool    only create on mobile      (default false)
        Draggable     = bool    can the button be dragged  (default true)
        ZIndex        = number                             (default 10)

    Returns a handle:
        Handle.Button                  -- the underlying TextButton
        Handle:SetVisible(bool)
        Handle:SetPosition(UDim2)
        Handle:SetOpenedIcon(name)
        Handle:SetClosedIcon(name)
        Handle:Destroy()
--]]
function Library:CustomToggleUi(Info)
    Info = Info or {}

    local Shape        = Info.Shape       or "Rounded"
    local Size         = Info.Size        or UDim2.fromOffset(40, 40)
    local Position     = Info.Position    or UDim2.fromOffset(6, 6)
    local OpenedIcon   = Info.OpenedIcon  or nil
    local ClosedIcon   = Info.ClosedIcon  or nil
    local OpenedColor  = Info.OpenedColor or nil   -- falls back to AccentColor
    local ClosedColor  = Info.ClosedColor or nil   -- falls back to MainColor
    local OnlyMobile   = Info.OnlyMobile  == true
    local Draggable    = Info.Draggable   ~= false  -- true by default
    local ZIdx         = Info.ZIndex      or 10

    -- OnlyMobile guard
    if OnlyMobile and not Library.IsMobile then
        return nil
    end

    local Handle = {}

    -- Resolve corner radius per shape
    local function GetCorner()
        ...
        elseif Shape == "Square" then
            ...
            return UDim.new(0, Library.CornerRadius + 4)
        end
    end
    ...
    local function ResolveIcon(Name)
        ...
        return Library:GetCustomIcon(Name)
    end

    local OpenedIconData = ResolveIcon(OpenedIcon)
    local ClosedIconData = ResolveIcon(ClosedIcon)

    -- Build button
    local Button = New("TextButton", {
        AnchorPoint        = Vector2.new(0, 0),
        BackgroundColor3   = ClosedColor or "MainColor",
        Position           = Position,
        Size               = Size,
        Text               = "",
        ZIndex             = ZIdx,
    ...
    New("UICorner", {
        CornerRadius = GetCorner(),
        Parent       = Button,
    })
    Library:AddOutline(Button)
    table.insert(Library.Scales, New("UIScale", { Parent = Button }))

    -- Icon image label (centred, switches on toggle state)
    ...
    -- Update icon and bg colour based on current toggle state
    local function Refresh()
        local IsOpen    = Library.Toggled
        local IconData  = IsOpen and OpenedIconData or ClosedIconData
        local BgColor   = IsOpen
            and (OpenedColor or Library.Scheme.AccentColor)
            or  (ClosedColor or Library.Scheme.MainColor)

        Button.BackgroundColor3 = BgColor

        if IconData then
            IconImage.Image              = IconData.Url
            IconImage.ImageRectOffset    = IconData.ImageRectOffset
            IconImage.ImageRectSize      = IconData.ImageRectSize
            IconImage.ImageColor3        = IsOpen
    ...
    -- Click: toggle UI + refresh icon
    Button.MouseButton1Click:Connect(function()
        Library:Toggle()
        -- small tween on click for feel
        TweenService:Create(Button, Library.TweenInfo, {
            BackgroundColor3 = Library.Toggled
                and (OpenedColor or Library.Scheme.AccentColor)
                or  (ClosedColor or Library.Scheme.MainColor),
        }):Play()
        Refresh()
    ...
    Button.MouseEnter:Connect(function()
        TweenService:Create(Button, Library.TweenInfo, {
            BackgroundColor3 = Library:GetLighterColor(
                Library.Toggled
                    and (OpenedColor or Library.Scheme.AccentColor)
                    or  (ClosedColor or Library.Scheme.MainColor)
            ),
    ...
    function Handle:SetPosition(Pos: UDim2)
        Button.Position = Pos
    end

    function Handle:SetOpenedIcon(Name: string)
        OpenedIcon     = Name
        OpenedIconData = ResolveIcon(Name)
        Refresh()
    end

    function Handle:SetClosedIcon(Name: string)
        ClosedIcon     = Name
        ClosedIconData = ResolveIcon(Name)
        Refresh()
    end

    function Handle:SetSize(NewSize: UDim2)
        Button.Size = NewSize
    end

    function Handle:SetShape(NewShape: string)
        Shape = NewShape
        local Corner = Button:FindFirstChildOfClass("UICorner")
        if Corner then
            Corner.CornerRadius = GetCorner()
        end
    end
...
-- Helper: attach a small icon to an element label row.
-- Returns the ImageLabel (or nil if icon invalid).
local function AttachElementIcon(Parent: GuiObject, IconName: string, Size: number?, OffsetX: number?, OffsetY: number?)
    ...
    local Img = New("ImageLabel", {
        ...
        Position = UDim2.new(0, OffsetX or 0, 0.5, OffsetY or 0),
...
function Library:SetIconModule(module: IconModule)
    ...
    CollapseIcon = Library:GetIcon("chevron-up")
end
...
do
    ...
    function Funcs:AddDivider(...)
        ...
        local Variant      = "Default"  -- "Default" | "Fade"
        local TextSize     = 12
        local Icon         = nil
        ...
        if typeof(Params) == "table" then
            ...
            Variant       = Params.Variant      or "Default"
            TextSize      = Params.TextSize     or 12
            Icon          = Params.Icon
        elseif typeof(Params) == "string" then
            Text = Params
        end

        local IsFade = Variant == "Fade"

        local Groupbox  = self
        local Container = Groupbox.Container

        local TotalHeight = 6 + MarginTop + MarginBottom
        if Text or Icon then TotalHeight = math.max(TotalHeight, 14 + MarginTop + MarginBottom) end
        ...
        -- Helper: build one line segment with optional fade gradient
        local function MakeLine(AnchorX, PosX, SizeX, FadeDir)
            local Line = New("Frame", {
                AnchorPoint     = Vector2.new(AnchorX, 0.5),
                BackgroundColor3 = "OutlineColor",
                Position        = UDim2.new(PosX, 0, 0.5, 0),
                Size            = UDim2.new(SizeX, 0, 0, 1),
                Parent          = InnerHolder,
            })

            if IsFade then
                -- FadeDir: "left" fades left-to-right (solid→transparent)
                --          "right" fades right-to-left (transparent→solid)
                --          "both" fades from edges inward (full width, no text)
                local Seq
                ...
                else -- "both" — single full-width line fading at both ends
                    New("UIGradient", {
                        Transparency = NumberSequence.new({
                            NumberSequenceKeypoint.new(0,    1),
        ...
        if Text or Icon then
            -- ── [ icon? ] [ Text ] ──
            -- measure combined content width
            local ContentW = 0
            local TempLabel = New("TextLabel", {
                AutomaticSize   = Enum.AutomaticSize.X,
                BackgroundTransparency = 1,
                Size            = UDim2.fromOffset(0, TextSize),
                Text            = Text or "",
                TextSize        = TextSize,
                TextTransparency = 0.5,
                Parent          = InnerHolder,
            ...
            if Text and Text ~= "" then
                TW = Library:GetTextBounds(Text, TempLabel.FontFace, TextSize, 9999)
            end
            local IW = (Icon and 14 or 0)
            local Gap = (Icon and Text and Text ~= "") and 4 or 0
            ContentW = TW + IW + Gap

            -- reposition label
            local HalfContent = ContentW / 2
            TempLabel.AnchorPoint = Vector2.new(0.5, 0.5)
            TempLabel.Position = UDim2.new(
                0.5,
                IW > 0 and (Gap + IW) / 2 or 0,
                0.5, 0
            )
            TempLabel.Size = UDim2.fromOffset(TW, TextSize)
            ...
            if Icon then
                ...
                if ParsedIcon then
                    New("ImageLabel", {
                        ...
                        Position             = UDim2.new(0.5, -(HalfContent) + IW / 2, 0.5, 0),
                        Size                 = UDim2.fromOffset(14, 14),
                        Parent               = InnerHolder,
                    })
                end
            ...
            local Pad = HalfContent + 8
            -- Left arm
            MakeLine(0, 0, 0.5, IsFade and "left" or nil)
                -- shrink to leave gap around content
            do
                local L = InnerHolder:FindFirstChildWhichIsA("Frame")
                if L then
                    L.Size = UDim2.new(0.5, -Pad, 0, 1)
                end
            end
            -- Right arm
            MakeLine(1, 1, 0.5, IsFade and "right" or nil)
            do
                local children = InnerHolder:GetChildren()
                local R = children[#children]
                if R:IsA("Frame") then
                    R.Size = UDim2.new(0.5, -Pad, 0, 1)
                end
            end
        else
            -- plain line, full width
            MakeLine(0, 0, 1, IsFade and "both" or nil)
        end
    ...
    function Funcs:AddLabel(...)
        ...
        -- Build the row: if icon provided, wrap icon + TextLabel in a Frame
        local LabelHolder
        local TextLabel

        if Data.Icon and not Data.DoesWrap then
            LabelHolder = New("Frame", {
                BackgroundTransparency = 1,
        ...
        function Label:SetText(Text: string)
            ...
            if Label.DoesWrap then
                ...
                if LabelHolder ~= TextLabel then
                    LabelHolder.Size = UDim2.new(1, 0, 0, Y + 4)
                end
            end
        ...
        if Label.DoesWrap then
            ...
            -- addon layout goes on TextLabel (the actual text node), not the wrapper frame
            New("UIListLayout", {
                FillDirection = Enum.FillDirection.Horizontal,
                HorizontalAlignment = Enum.HorizontalAlignment.Right,
                Padding = UDim.new(0, 6),
    ...
    function Funcs:AddButton(...)
        ...
        local function CreateButton(Button)
            ...
            -- Icon + label layout inside button
            ...
            if Button.Icon then
                local ParsedIcon = Library:GetCustomIcon(Button.Icon)
                if ParsedIcon then
                    New("ImageLabel", {
                        Image = ParsedIcon.Url,
                        ImageColor3 = ParsedIcon.Custom and "WhiteColor" or (Button.Risky and "RedColor" or "AccentColor"),
                        ImageRectOffset = ParsedIcon.ImageRectOffset,
                        ImageRectSize = ParsedIcon.ImageRectSize,
            ...
            local BtnTextLabel = New("TextLabel", {
                AutomaticSize = Enum.AutomaticSize.X,
                BackgroundTransparency = 1,
                Size = UDim2.fromOffset(0, 14),
                Text = Button.Text,
        ...
        local function InitEvents(Button)
            local TL = Button.TextLabel -- child TextLabel inside BtnContent
            Button.Base.MouseEnter:Connect(function()
                if Button.Disabled then return end
                Button.Tween = TweenService:Create(TL, Library.TweenInfo, { TextTransparency = 0 })
                Button.Tween:Play()
            end)
            Button.Base.MouseLeave:Connect(function()
                if Button.Disabled then return end
                Button.Tween = TweenService:Create(TL, Library.TweenInfo, { TextTransparency = 0.4 })
                Button.Tween:Play()
            end)
            ...
            Button.Base.MouseButton1Click:Connect(function()
                ...
                if Button.DoubleClick then
                    ...
                    TL.Text = "Are you sure?"
                    TL.TextColor3 = Library.Scheme.AccentColor
                    Library.Registry[TL] = Library.Registry[TL] or {}
                    Library.Registry[TL].TextColor3 = "AccentColor"

                    local Clicked = WaitForEvent(Button.Base.MouseButton1Click, 0.5)

                    TL.Text = Button.Text
                    TL.TextColor3 = Button.Risky and Library.Scheme.RedColor or Library.Scheme.FontColor
                    Library.Registry[TL].TextColor3 = Button.Risky and "RedColor" or "FontColor"
        ...
        function Button:AddButton(...)
            ...
            function SubButton:SetText(Text: string)
                SubButton.Text = Text
                if SubButton.TextLabel then SubButton.TextLabel.Text = Text end
            end
            ...
            if SubButton.Risky and SubButton.TextLabel then
                SubButton.TextLabel.TextColor3 = Library.Scheme.RedColor
                Library.Registry[SubButton.TextLabel] = Library.Registry[SubButton.TextLabel] or {}
                Library.Registry[SubButton.TextLabel].TextColor3 = "RedColor"
            end
        ...
        function Button:SetText(Text: string)
            Button.Text = Text
            if Button.TextLabel then Button.TextLabel.Text = Text end
        end
        ...
        if Button.Risky and Button.TextLabel then
            Button.TextLabel.TextColor3 = Library.Scheme.RedColor
            Library.Registry[Button.TextLabel] = Library.Registry[Button.TextLabel] or {}
            Library.Registry[Button.TextLabel].TextColor3 = "RedColor"
        end
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        -- Icon support for checkbox
        local CheckboxIconOffset = 0
        if Info.Icon then
            local ParsedIcon = Library:GetCustomIcon(Info.Icon)
            if ParsedIcon then
                CheckboxIconOffset = 18
                New("ImageLabel", {
                    AnchorPoint = Vector2.new(0, 0.5),
                    Image = ParsedIcon.Url,
                    ImageColor3 = ParsedIcon.Custom and "WhiteColor" or "AccentColor",
        ...
        local Label = New("TextLabel", {
            BackgroundTransparency = 1,
            Position = UDim2.fromOffset(26 + CheckboxIconOffset, 0),
            Size = UDim2.new(1, -26 - CheckboxIconOffset, 1, 0),
            Text = Toggle.Text,
            TextSize = 14,
            TextTransparency = 0.4,
            TextTruncate = Enum.TextTruncate.AtEnd,
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local Label = New("TextLabel", {
            BackgroundTransparency = 1,
            Position = UDim2.fromOffset(ToggleIconOffset, 0),
            Size = UDim2.new(1, -40 - ToggleIconOffset, 1, 0),
            Text = Toggle.Text,
            TextSize = 14,
            TextTransparency = 0.4,
            TextTruncate = Enum.TextTruncate.AtEnd,
        ...
        if not Info.Compact then
            ...
            if Info.Icon then
                ...
                if ParsedIcon then
                    ...
                    SliderLabel.Position = UDim2.fromOffset(16, 0)
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        -- Icon support for dropdown
        if Info.Icon and Info.Text then
            local ParsedIcon = Library:GetCustomIcon(Info.Icon)
            if ParsedIcon then
                New("ImageLabel", {
                    AnchorPoint = Vector2.new(0, 0),
    ...
    --[[
        Funcs:AddPanelButton(Info)

        Adds a button row in a groupbox. Clicking it toggles a floating draggable
        panel that can have any BaseGroupbox elements added to it (AddToggle, AddSlider…).

        Info fields:
            Text         = string   — button label and panel title   (default "Panel")
            Icon         = string   — lucide icon name / asset url   (default "settings")
            PanelWidth   = number   — width of the floating panel    (default 220)
            Visible      = bool                                       (default true)

        Returns a Panel object with all BaseGroupbox methods (AddToggle, AddSlider, etc.)
        and also:
            Panel:SetVisible(bool)
            Panel:Close()
            Panel:Open()
    --]]
    function Funcs:AddPanelButton(Info)
        Info = Info or {}
        local Text       = Info.Text       or "Panel"
        local IconName   = Info.Icon       or "settings"
        local PanelWidth = Info.PanelWidth or 220
        ...
        New("UICorner", { CornerRadius = UDim.new(0, Library.CornerRadius), Parent = PanelFrame })
        Library:AddOutline(PanelFrame)
        table.insert(Library.Scales, New("UIScale", { Parent = PanelFrame }))
        ...
        local TitleIcon = Library:GetCustomIcon(IconName)
        if TitleIcon then
            New("ImageLabel", {
                AnchorPoint = Vector2.new(0, 0.5),
                Image = TitleIcon.Url,
        ...
        New("TextLabel", {
            BackgroundTransparency = 1,
            Position = UDim2.fromOffset(TitleIcon and 28 or 10, 0),
            Size = UDim2.new(1, -(TitleIcon and 36 or 14), 1, 0),
            Text = Text,
            TextSize = 13,
            TextTruncate = Enum.TextTruncate.AtEnd,
            TextXAlignment = Enum.TextXAlignment.Left,
        ...
        Library:MakeLine(PanelFrame, {
            Position = UDim2.fromOffset(0, 30),
            Size = UDim2.new(1, 0, 0, 1),
        })
        Library:MakeDraggable(PanelFrame, TitleBar, true)
        ...
        PanelList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            PanelFrame.Size = UDim2.fromOffset(
                PanelWidth,
                math.floor(PanelList.AbsoluteContentSize.Y / Library.DPIScale) + 45
            )
        end)

        -- ── Button row in the groupbox ────────────────────────────────────
        local Holder = New("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, 21),
            Visible = Visible,
        ...
        local BtnIcon = Library:GetCustomIcon(IconName)
        if BtnIcon then
            New("ImageLabel", {
                Image = BtnIcon.Url,
                ImageColor3 = "AccentColor",
        ...
        New("UIStroke", { Color = "OutlineColor", Parent = BtnBase })

        -- Hover effects
        BtnBase.MouseEnter:Connect(function()
            TweenService:Create(BtnLabel, Library.TweenInfo, { TextTransparency = 0 }):Play()
        end)
        BtnBase.MouseLeave:Connect(function()
            TweenService:Create(BtnLabel, Library.TweenInfo, { TextTransparency = 0.4 }):Play()
        end)

        -- Panel positioning helper
        local function PositionPanel()
            local S = ScreenGui.AbsoluteSize
            local AbsPos = BtnBase.AbsolutePosition
            local AbsSize = BtnBase.AbsoluteSize
            local PX = math.floor(AbsPos.X / Library.DPIScale)
            local PY = math.floor((AbsPos.Y + AbsSize.Y) / Library.DPIScale) + 2
            -- Keep within screen
            local PanelW = PanelWidth * Library.DPIScale
            if PX + PanelW > S.X then
                PX = math.floor((S.X - PanelW) / Library.DPIScale)
            end
            PanelFrame.Position = UDim2.fromOffset(PX, PY)
        end
        ...
        local function OpenPanel()
            ...
            TweenService:Create(PanelFrame, Library.TweenInfo, {
        ...
        -- ── Panel object (supports all BaseGroupbox element methods) ──────
        local Panel = {
            Holder = Holder,
        ...
        function Panel:Resize()
            -- No-op: panel auto-sizes via AbsoluteContentSize signal
        end

        function Panel:SetVisible(V: boolean)
            Panel.Visible = V
            Holder.Visible = V
            if not V then ClosePanel() end
            Groupbox:Resize()
        end
...
function Library:CreateWindow(WindowInfo)
    ...
    if Library.IsMobile and WindowInfo.AutoResizeMobile then
    ...
    elseif not Library.IsMobile then
        Library.OriginalMinSize =
            Vector2.new(math.min(Library.OriginalMinSize.X, MaxX), math.min(Library.OriginalMinSize.Y, MaxY))
        Library.MinSize = Library.OriginalMinSize
    ...
    -- AutoResizeMobile = false on mobile: use WindowInfo.Size exactly as provided
    ...
    function Window:AddTab(...)
        ...
        local TabLeft
        local TabRight
        local TabSingle  -- single-canvas mode
        ...
        do
            ...
            if SingleCanvas then
                -- Single full-width column
                TabSingle = New("ScrollingFrame", {
                    AutomaticCanvasSize = Enum.AutomaticSize.Y,
                    BackgroundTransparency = 1,
                    CanvasSize = UDim2.fromScale(0, 0),
                ...
                do
                    New("Frame", { BackgroundTransparency = 1, LayoutOrder = -1, Parent = TabSingle })
                    New("Frame", { BackgroundTransparency = 1, LayoutOrder = 1, Parent = TabSingle })
                end
                TabLeft = TabSingle
                TabRight = TabSingle
            else
                -- Dual-column layout (default)
                ...
                New("UIPadding", {
                    PaddingBottom = UDim.new(0, 2), PaddingLeft = UDim.new(0, 2),
                    PaddingRight = UDim.new(0, 2), PaddingTop = UDim.new(0, 2),
                    Parent = TabLeft,
                })
                do
        ...
        function Tab:RefreshSides()
            ...
            if SingleCanvas and TabSingle then
                -- Single canvas: one full-width column
                TabSingle.Position = UDim2.new(0, 0, 0, Offset)
                TabSingle.Size = UDim2.new(1, 0, 1, -Offset)
            else
        ...
        function Tab:AddGroupbox(Info)
            ...
            -- Collapsible state
            local IsCollapsed = Info.DefaultCollapsed or false
            local Collapsible = Info.Collapsible ~= false and (Info.Collapsible or false)
            ...
            do
                ...
                -- Header button (clickable if collapsible)
                local HeaderButton = New(Collapsible and "TextButton" or "Frame", {
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1, 0, 0, 34),
                    Text = "",
                    Parent = GroupboxHolder,
                ...
                -- Header hover tint when collapsible
                if Collapsible then
                    HeaderButton.MouseEnter:Connect(function()
                        TweenService:Create(GroupboxHolder, Library.TweenInfo, {
                            BackgroundColor3 = Library:GetBetterColor(Library.Scheme.BackgroundColor, 3),
                        }):Play()
                    end)
                ...
                DividerLine = Library:MakeLine(GroupboxHolder, {
                    Position = UDim2.fromOffset(0, 34),
                    Size = UDim2.new(1, 0, 0, 1),
                })
                ...
                GroupboxLabel = New("TextLabel", {
                    ...
                    Size = UDim2.new(1, -(iconOffset + (Collapsible and 28 or 12)), 0, 34),
                    Text = Info.Name,
                    TextSize = 15,
                    TextTruncate = Enum.TextTruncate.AtEnd,
                    TextXAlignment = Enum.TextXAlignment.Left,
                ...
                -- Collapse arrow icon (only when collapsible)
                if Collapsible and CollapseIcon then
            ...
            local function GetExpandedHeight()
                return (GroupboxList.AbsoluteContentSize.Y / Library.DPIScale) + 49
            end
            ...
            function Groupbox:SetCollapsed(State: boolean)
                ...
                if State then
                    -- Hide content first, then shrink holder
                    DividerLine.Visible = false
                    GroupboxContainer.Visible = false
                    TweenService:Create(GroupboxHolder, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Size = UDim2.new(1, 0, 0, 34),
                    }):Play()
                else
                    -- Grow holder first, then reveal content
                    DividerLine.Visible = true
                    TweenService:Create(GroupboxHolder, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Size = UDim2.new(1, 0, 0, GetExpandedHeight()),
                    }):Play()
            ...
            -- Wire up header click
            if Collapsible then
                local HeaderBtn = GroupboxHolder:FindFirstChildWhichIsA("TextButton")
                if HeaderBtn then
            ...
            -- Use name as key if non-empty, otherwise auto-index to avoid collisions
        ...
        function Tab:AddLeftGroupbox(Name, Info)
            -- Accept both string shorthand and table
            if typeof(Name) == "table" then
                Name.Side = 1
                return Tab:AddGroupbox(Name)
            end
            local IconName = typeof(Info) == "string" and Info or (typeof(Info) == "table" and Info.IconName or nil)
            local Collapsible = typeof(Info) == "table" and Info.Collapsible or nil
            local DefaultCollapsed = typeof(Info) == "table" and Info.DefaultCollapsed or nil
            return Tab:AddGroupbox({ Side = 1, Name = Name, IconName = IconName, Collapsible = Collapsible, DefaultCollapsed = DefaultCollapsed })
        end
        ...
        function Tab:AddRightGroupbox(Name, Info)
            ...
            local Collapsible = typeof(Info) == "table" and Info.Collapsible or nil
            local DefaultCollapsed = typeof(Info) == "table" and Info.DefaultCollapsed or nil
            return Tab:AddGroupbox({ Side = 2, Name = Name, IconName = IconName, Collapsible = Collapsible, DefaultCollapsed = DefaultCollapsed })
        end
        ...
        function Tab:AddTabbox(Info)
            ...
            local TabboxContent  -- wraps buttons + content when collapsible

            local IsTabboxCollapsed = Info.DefaultCollapsed or false
            local TabboxCollapsible = Info.Collapsible or false
            ...
            do
                ...
                -- Optional collapse header above the tab buttons
                if TabboxCollapsible then
                    local CollapseHeader = New("TextButton", {
                        BackgroundTransparency = 1,
                    ...
                    local CollapseTitle = New("TextLabel", {
                        BackgroundTransparency = 1,
                        Position = UDim2.fromOffset(0, 0),
                    ...
                    New("UIPadding", { PaddingLeft = UDim.new(0, 12), Parent = CollapseTitle })

                    if CollapseIcon then
                        TabboxCollapseArrow = New("ImageLabel", {
                            AnchorPoint = Vector2.new(1, 0.5),
                            Image = CollapseIcon.Url,
                            ImageColor3 = "FontColor",
                            ImageRectOffset = CollapseIcon.ImageRectOffset,
                            ...
                            ImageTransparency = 0.5,
                            Position = UDim2.new(1, -10, 0.5, 0),
                            Rotation = IsTabboxCollapsed and 180 or 0,
                            Size = UDim2.fromOffset(14, 14),
                            Parent = CollapseHeader,
                        })
                    end
                    -- Hover tint
                    CollapseHeader.MouseEnter:Connect(function()
                        TweenService:Create(TabboxHolder, Library.TweenInfo, {
                            BackgroundColor3 = Library:GetBetterColor(Library.Scheme.BackgroundColor, 3),
                        }):Play()
                    end)
                    CollapseHeader.MouseLeave:Connect(function()
                        TweenService:Create(TabboxHolder, Library.TweenInfo, {
                            BackgroundColor3 = Library.Scheme.BackgroundColor,
                        }):Play()
                    end)
                    ...
                    -- CollapseHeader click wired after Tabbox table is built (below)
                end

                TabboxButtons = New("Frame", {
                    BackgroundTransparency = 1,
                    Position = TabboxCollapsible and UDim2.fromOffset(0, 29) or UDim2.fromOffset(0, 0),
                    Size = UDim2.new(1, 0, 0, 34),
                    Visible = not IsTabboxCollapsed,
                    Parent = TabboxHolder,
                })
                New("UIListLayout", {
                    FillDirection = Enum.FillDirection.Horizontal,
            ...
            local Tabbox = {
                ...
                Holder = TabboxHolder,
                Tabs = {},
                Collapsed = IsTabboxCollapsed,
            }

            -- Tabbox collapse helpers
            function Tabbox:SetCollapsed(State: boolean)
                Tabbox.Collapsed = State
                if TabboxCollapseArrow then
                    TweenService:Create(TabboxCollapseArrow, Library.TweenInfo, {
                        Rotation = State and 180 or 0,
                    }):Play()
                end
                if State then
                    TabboxButtons.Visible = false
                    -- hide active tab content
                    if Tabbox.ActiveTab then
                        Tabbox.ActiveTab.Container.Visible = false
                    end
                    TweenService:Create(TabboxHolder, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Size = UDim2.new(1, 0, 0, TabboxCollapsible and 29 or 34),
                    }):Play()
                else
            ...
            function Tabbox:ToggleCollapse()
                Tabbox:SetCollapsed(not Tabbox.Collapsed)
            end

            -- Wire collapse click
            if TabboxCollapsible then
                local Header = TabboxHolder:FindFirstChildWhichIsA("TextButton")
                if Header then
                    Header.MouseButton1Click:Connect(function()
                        Tabbox:ToggleCollapse()
                    end)
                end
            end
            ...
            function Tabbox:AddTab(Name, IconName)
                ...
                -- Content container offset: extra 29px if collapsible header present
                local ContentOffset = TabboxCollapsible and (29 + 35) or 35
                local Container = New("Frame", {
                    BackgroundTransparency = 1,
                ...
                function Tab:Resize()
                    ...
                    local HeaderExtra = TabboxCollapsible and 29 or 0
                    TabboxHolder.Size = UDim2.new(1, 0, 0, (List.AbsoluteContentSize.Y / Library.DPIScale) + 49 + HeaderExtra)
                end
        ...
        -- AddSingleGroupbox: full-width groupbox, useful in Single-canvas tabs
        -- Also usable in dual-column tabs (placed on left side spanning full width)
        function Tab:AddSingleGroupbox(Name, Info)
            if typeof(Name) == "table" then
                Name.Side = 1
                Name.FullWidth = true
                return Tab:AddGroupbox(Name)
            ...
            local Collapsible = typeof(Info) == "table" and Info.Collapsible or nil
            local DefaultCollapsed = typeof(Info) == "table" and Info.DefaultCollapsed or nil
            return Tab:AddGroupbox({ Side = 1, Name = Name, IconName = IconName, Collapsible = Collapsible, DefaultCollapsed = DefaultCollapsed, FullWidth = true })
        end
        ...
        function Tab:Show()
            ...
            -- Respect SetUITransparency: active tab background uses MainColor role (0.85)
            local HighlightAlpha = Library.UITransparency * 0.85
            TweenService:Create(TabButton, Library.TweenInfo, {
                BackgroundTransparency = HighlightAlpha,
            }):Play()
            TweenService:Create(TabLabel, Library.TweenInfo, {
                TextTransparency = 0,
            }):Play()
            ...
            -- Fade content in smoothly via GroupTransparency
            TabContainer.Visible = true
            TabContainer.GroupTransparency = 0.4
            TweenService:Create(TabContainer, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                GroupTransparency = 0,
            }):Play()
    ...
    function Window:AddKeyTab(...)
        ...
        -- Accent active indicator bar on left edge of tab button
        local TabActiveBar = New("Frame", {
            AnchorPoint = Vector2.new(0, 0.5),
            BackgroundColor3 = "AccentColor",
            BackgroundTransparency = 1,
            Position = UDim2.new(0, 0, 0.5, 0),
    ...
    function Library:Toggle(Value: boolean?)
        ...
        if Library.Toggled then
            ...
            -- Smooth open: scale up from 0.95 and fade in
            local UIScaleInst = MainFrame:FindFirstChildWhichIsA("UIScale")
            if UIScaleInst then
                UIScaleInst.Scale = Library.DPIScale * 0.95
                TweenService:Create(UIScaleInst, TweenInfo.new(0.14, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    Scale = Library.DPIScale,
                }):Play()
            end
            MainFrame.GroupTransparency = 0.25
            TweenService:Create(MainFrame, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                GroupTransparency = 0,
            }):Play()
        else
            -- Smooth close: fade out, then hide
    ...
    if Library.IsMobile and not WindowInfo.DisableToggle then
    ...
    Library:GiveSignal(UserInputService.InputBegan:Connect(function(Input: InputObject)
        ...
        if WindowInfo.DisableToggle then
            return
        end
