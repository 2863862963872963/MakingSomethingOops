...
local Library = {
    ...
    Style = {
        ...
        TabBarTransparency    = 0,      -- 0 = fully opaque, 1 = fully transparent (sidebar)
        BlurEnabled           = false,  -- Blur effect behind the window via Lighting
        BlurIntensity         = 10,     -- Blur size (1–56)
    },

    GroupboxHolders = {},  -- tracked for live transparency changes
    MainFrame = nil,       -- set by CreateWindow for live transparency changes
    TabBarFrame = nil,     -- set by CreateWindow for live tab bar transparency
    BlurEffect  = nil,     -- BlurEffect instance managed by SetStyle
}
...
function Library:SetStyle(StyleOverrides)
    ...
    if StyleOverrides.TabBarTransparency ~= nil and Library.TabBarFrame then
        TweenService:Create(Library.TabBarFrame, Library.TweenInfo, {
            BackgroundTransparency = Library.Style.TabBarTransparency,
        }):Play()
    end

    if StyleOverrides.BlurEnabled ~= nil or StyleOverrides.BlurIntensity ~= nil then
        if Library.Style.BlurEnabled then
            if not Library.BlurEffect then
                pcall(function()
                    local Blur = Instance.new("BlurEffect")
                    Blur.Size = Library.Style.BlurIntensity
                    Blur.Parent = game:GetService("Lighting")
                    Library.BlurEffect = Blur
                end)
            elseif Library.BlurEffect then
                pcall(function()
                    Library.BlurEffect.Size = Library.Style.BlurIntensity
                end)
            end
        else
...
do
    ...
    function Funcs:AddDivider(...)
        ...
        if typeof(Params) == "table" then
            --// Delegate to AddFadeDivider if Variant = "Fade"
            if Params.Variant == "Fade" then
                return self:AddFadeDivider(Params)
            end
            Text = Params.Text
...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddSettingPage()
            ...
            RightVisual:AddFadeDivider("Transparency")

            RightVisual:AddSlider("_Settings_TabBarOpacity", {
                Text     = "Tab Bar Opacity",
                Icon     = "panel-left",
                Default  = math.floor((1 - Library.Style.TabBarTransparency) * 100),
                Min      = 0,
                Max      = 100,
                Rounding = 0,
                Suffix   = "%",
                Callback = function(Value)
                    Library:SetStyle({ TabBarTransparency = 1 - Value / 100 })
                end,
            })

            RightVisual:AddFadeDivider("Blur")

            RightVisual:AddToggle("_Settings_BlurEnabled", {
                Text    = "Window Blur",
                Icon    = "sparkles",
                Default = Library.Style.BlurEnabled,
                Callback = function(Value)
                    Library:SetStyle({ BlurEnabled = Value })
                end,
            })

            RightVisual:AddSlider("_Settings_BlurIntensity", {
                Text     = "Blur Intensity",
                Icon     = "eye",
                Default  = Library.Style.BlurIntensity,
                Min      = 1,
                Max      = 56,
                Rounding = 0,
                Callback = function(Value)
                    Library.Style.BlurIntensity = Value
                    if Library.BlurEffect then
                        pcall(function() Library.BlurEffect.Size = Value end)
                    end
                end,
            })
