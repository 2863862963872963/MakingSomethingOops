...
do
    ...
    function Funcs:AddDivider(...)
        ...
        if Text or Icon then
            local DividerIconData = Icon and Library:GetCustomIcon(Icon) or nil
            local IconWidth = DividerIconData and 18 or 0

            if DividerIconData then
                New("ImageLabel", {
                    AnchorPoint = Vector2.new(0.5, 0.5),
                    Image = DividerIconData.Url,
                    ImageColor3 = DividerIconData.Custom and "WhiteColor" or "FontColor",
                    ImageRectOffset = DividerIconData.ImageRectOffset,
                    ImageRectSize = DividerIconData.ImageRectSize,
                    ImageTransparency = 0.5,
                    Position = UDim2.fromScale(0.5, 0.5),
                    Size = UDim2.fromOffset(12, 12),
                    ZIndex = InnerHolder.ZIndex + 1,
                    Parent = InnerHolder,
                })
            end
            ...
            if DisplayText ~= "" then
                TextLabel = New("TextLabel", {
                    ...
                    AutomaticSize = Enum.AutomaticSize.X,
                    BackgroundTransparency = 1,
                    Position = UDim2.new(0.5, DividerIconData and (IconWidth / 2) or 0, 0.5, 0),
                    Size = UDim2.fromScale(0, 1),
                    Text = DisplayText,
                    TextSize = 14,
                    TextTransparency = 0.5,
            ...
            local function GetHalfWidth()
                local TextW = TextLabel and TextLabel.AbsoluteSize.X or 0
                return (TextW + IconWidth) // 2 + 10
            end

            local function LayoutLines()
                local HalfW = GetHalfWidth()
            ...
            if TextLabel then
                local X, _ = Library:GetTextBounds(DisplayText, TextLabel.FontFace, TextLabel.TextSize, TextLabel.AbsoluteSize.X)
                local SizeX = (X + IconWidth) // 2 + 10
                New("Frame", {
                    AnchorPoint = Vector2.new(0, 0.5),
                    BackgroundColor3 = "MainColor",
                    BorderColor3 = "OutlineColor",
    ...
    --// Accent Divider — theme-colored gradient line fading in both directions \\--
    function Funcs:AddAccentDivider(...)
        local Params = select(1, ...)
        local Text
        local Icon
        local MarginTop  = 0
        local MarginBottom = 0
        local Height = 2      -- line thickness (px)
        ...
        local HasContent = (Text and Text ~= "") or Icon
        local TotalH     = (HasContent and 14 or Height) + MarginTop + MarginBottom
        ...
        --// Helper: build a gradient line frame \\--
        local function MakeGradientLine(AnchorX, PosX, SizeXScale, SizeXOffset, FlipGradient)
            local Line = New("Frame", {
                AnchorPoint           = Vector2.new(AnchorX, 0.5),
                BackgroundColor3      = "AccentColor",
                BorderSizePixel       = 0,
                Position              = UDim2.new(PosX, 0, 0.5, 0),
                Size                  = UDim2.new(SizeXScale, SizeXOffset, 0, Height),
                Parent                = InnerHolder,
            })
            New("UICorner", {
                CornerRadius = UDim.new(1, 0),
            ...
            -- Transparency gradient: opaque at inner edge, transparent at outer edge
            local Gradient = Instance.new("UIGradient")
            Gradient.Transparency = NumberSequence.new({
                NumberSequenceKeypoint.new(0, FlipGradient and 0 or 1),
                NumberSequenceKeypoint.new(1, FlipGradient and 1 or 0),
            })
        ...
        if HasContent then
            --// Content (icon + text) centred, lines on either side \\--

            local AccentIconData = Icon and Library:GetCustomIcon(Icon) or nil
            local IconW          = AccentIconData and 18 or 0
            local HasText        = Text and Text ~= ""

            --// Content holder — horizontally centred \\--
            ...
            if AccentIconData then
                local IconLabel = New("ImageLabel", {
                    Image            = AccentIconData.Url,
                    ImageColor3      = AccentIconData.Custom and "WhiteColor" or "AccentColor",
                    ImageRectOffset  = AccentIconData.ImageRectOffset,
                    ImageRectSize    = AccentIconData.ImageRectSize,
                    ImageTransparency = 0,
                    Size             = UDim2.fromOffset(12, 12),
                    Parent           = ContentHolder,
                })
            ...
            --// Left line: anchor right edge at left side of content \\--
            local LeftLine, _ = MakeGradientLine(1, 0, 0.5, -8, false)
            --// Right line: anchor left edge at right side of content \\--
            local RightLine, _ = MakeGradientLine(0, 0.5, 0.5, -8, true)

            -- Reposition lines once the content frame has a real size
            task.defer(function()
                if not (ContentHolder and ContentHolder.Parent) then return end
                local HalfContent = (ContentHolder.AbsoluteSize.X / Library.DPIScale) / 2 + 6
                LeftLine.Size  = UDim2.new(0.5, -HalfContent, 0, Height)
                RightLine.Size = UDim2.new(0.5, -HalfContent, 0, Height)
            end)
            ...
            --// Plain gradient line — full width, fades from edges to bright centre \\--
            -- Left half: transparent (left) → opaque (right)
            MakeGradientLine(0, 0, 0.5, 0, true)
            -- Right half: opaque (left) → transparent (right)
            MakeGradientLine(1, 1, 0.5, 0, false)
        end
        ...
        local LabelIconData = Data.Icon and Library:GetCustomIcon(Data.Icon) or nil
        local LabelIconOffset = LabelIconData and 20 or 0
        ...
        if LabelIconData then
            New("ImageLabel", {
                Image = LabelIconData.Url,
                ImageColor3 = LabelIconData.Custom and "WhiteColor" or "FontColor",
                ImageRectOffset = LabelIconData.ImageRectOffset,
                ImageRectSize = LabelIconData.ImageRectSize,
                ImageTransparency = 0,
                AnchorPoint = Vector2.new(0, 0.5),
                Position = UDim2.fromScale(0, 0.5),
                Size = UDim2.fromOffset(14, 14),
        ...
        local TextLabel = New("TextLabel", {
            BackgroundTransparency = 1,
            Position = UDim2.fromOffset(LabelIconOffset, 0),
            Size = UDim2.new(1, -LabelIconOffset, 1, 0),
            Text = Label.Text,
            TextSize = Data.Size,
        ...
        function Label:SetText(Text: string)
            ...
            if Label.DoesWrap then
                ...
                TextLabel.Size = UDim2.new(1, -LabelIconOffset, 0, Y + 4)
            end
    ...
    function Funcs:AddButton(...)
        ...
        local ButtonIconData = Info.Icon and Library:GetCustomIcon(Info.Icon) or nil
        ...
        local function CreateButton(Button)
            ...
            if ButtonIconData then
                New("ImageLabel", {
                    AnchorPoint = Vector2.new(0, 0.5),
                    Image = ButtonIconData.Url,
                    ImageColor3 = ButtonIconData.Custom and "WhiteColor" or "FontColor",
                    ImageRectOffset = ButtonIconData.ImageRectOffset,
                    ImageRectSize = ButtonIconData.ImageRectSize,
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        local CheckboxIconData = Info.Icon and Library:GetCustomIcon(Info.Icon) or nil
        local CheckboxIconOffset = CheckboxIconData and 20 or 0

        if CheckboxIconData then
            New("ImageLabel", {
                Image = CheckboxIconData.Url,
                ImageColor3 = CheckboxIconData.Custom and "WhiteColor" or "FontColor",
                ImageRectOffset = CheckboxIconData.ImageRectOffset,
                ImageRectSize = CheckboxIconData.ImageRectSize,
                ImageTransparency = 0.4,
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local ToggleIconData = Info.Icon and Library:GetCustomIcon(Info.Icon) or nil
        local IconOffset = ToggleIconData and 20 or 0
        ...
        local Label = New("TextLabel", {
            ...
            Size = UDim2.new(1, -40 - IconOffset, 1, 0),
            Text = Toggle.Text,
            TextSize = 14,
            TextTransparency = 0.4,
            TextXAlignment = Enum.TextXAlignment.Left,
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        local InputIconData = Info.Icon and Library:GetCustomIcon(Info.Icon) or nil
        local InputIconOffset = InputIconData and 20 or 0

        if InputIconData then
            New("ImageLabel", {
                Image = InputIconData.Url,
                ImageColor3 = InputIconData.Custom and "WhiteColor" or "FontColor",
                ImageRectOffset = InputIconData.ImageRectOffset,
                ImageRectSize = InputIconData.ImageRectSize,
                ImageTransparency = 0,
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        local SliderIconData = Info.Icon and Library:GetCustomIcon(Info.Icon) or nil
        local SliderIconOffset = SliderIconData and 20 or 0

        local SliderLabel
        if not Info.Compact then
            if SliderIconData then
                New("ImageLabel", {
                    Image = SliderIconData.Url,
                    ImageColor3 = SliderIconData.Custom and "WhiteColor" or "FontColor",
                    ImageRectOffset = SliderIconData.ImageRectOffset,
                    ImageRectSize = SliderIconData.ImageRectSize,
                    ImageTransparency = 0,
                    Position = UDim2.fromOffset(0, 0),
                    Size = UDim2.fromOffset(14, 14),
                    Parent = Holder,
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local DropdownIconData = Info.Icon and Library:GetCustomIcon(Info.Icon) or nil
        local DropdownIconOffset = DropdownIconData and 20 or 0

        if DropdownIconData then
            New("ImageLabel", {
                Image = DropdownIconData.Url,
                ImageColor3 = DropdownIconData.Custom and "WhiteColor" or "FontColor",
                ImageRectOffset = DropdownIconData.ImageRectOffset,
                ImageRectSize = DropdownIconData.ImageRectSize,
                ImageTransparency = 0,
                Position = UDim2.fromOffset(0, 0),
                Size = UDim2.fromOffset(14, 14),
        ...
        local Label = New("TextLabel", {
            BackgroundTransparency = 1,
            Position = UDim2.fromOffset(DropdownIconOffset, 0),
            Size = UDim2.new(1, -DropdownIconOffset, 0, 14),
            Text = Dropdown.Text,
            TextSize = 14,
            TextXAlignment = Enum.TextXAlignment.Left,
            Visible = not not Info.Text,
...
Library.Themes = {
    ...
    Ocean = {
        BackgroundColor = Color3.fromRGB(8, 18, 30),
        MainColor       = Color3.fromRGB(14, 30, 50),
        AccentColor     = Color3.fromRGB(0, 180, 220),
        OutlineColor    = Color3.fromRGB(20, 50, 80),
        FontColor       = Color3.fromRGB(200, 235, 255),
        IsLightTheme    = false,
    },
    Ember = {
        BackgroundColor = Color3.fromRGB(18, 10, 8),
        MainColor       = Color3.fromRGB(30, 16, 12),
        AccentColor     = Color3.fromRGB(255, 110, 40),
        OutlineColor    = Color3.fromRGB(55, 28, 18),
        FontColor       = Color3.fromRGB(255, 235, 215),
        IsLightTheme    = false,
    },
    Mint = {
        BackgroundColor = Color3.fromRGB(8, 18, 14),
        MainColor       = Color3.fromRGB(12, 28, 22),
        AccentColor     = Color3.fromRGB(50, 210, 150),
        OutlineColor    = Color3.fromRGB(20, 55, 40),
        FontColor       = Color3.fromRGB(210, 255, 235),
        IsLightTheme    = false,
    },
}
...
function Library:SetTheme(ThemeName: string)
    ...
    assert(Theme, string.format("Unknown theme %q. Valid themes: Dark, Light, Ocean, Ember, Mint", tostring(ThemeName)))
    ...
    Library.Scheme.OutlineColor    = Theme.OutlineColor
    Library.Scheme.FontColor       = Theme.FontColor
    Library.IsLightTheme           = Theme.IsLightTheme
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                --// Collapse Button \\--
                CollapseChevronIcon = Library:GetIcon("chevron-down")
                CollapseButton = New("TextButton", {
                    AnchorPoint = Vector2.new(1, 0.5),
                    BackgroundTransparency = 1,
                    Position = UDim2.new(1, -8, 0, 17),
                    Size = UDim2.fromOffset(20, 20),
                    Text = CollapseChevronIcon and "" or "▼",
                    TextColor3 = "FontColor",
                    TextSize = 10,
                    TextTransparency = 0.5,
                ...
                if CollapseChevronIcon then
                    New("ImageLabel", {
                        Image = CollapseChevronIcon.Url,
                        ImageColor3 = "FontColor",
                        ImageRectOffset = CollapseChevronIcon.ImageRectOffset,
                        ImageRectSize = CollapseChevronIcon.ImageRectSize,
                        ImageTransparency = 0.5,
                        Size = UDim2.fromScale(1, 1),
            ...
            function Groupbox:SetCollapsed(State: boolean)
                ...
                local CollapseIcon = Library:GetIcon(State and "chevron-right" or "chevron-down")

                if CollapseChevronIcon then
                    local IconLabel = CollapseButton:FindFirstChildWhichIsA("ImageLabel")
                    if IconLabel and CollapseIcon then
                        IconLabel.ImageRectOffset = CollapseIcon.ImageRectOffset
                        IconLabel.ImageRectSize = CollapseIcon.ImageRectSize
                    end
                else
                    CollapseButton.Text = State and "▶" or "▼"
                end

                if State then
                    TweenService:Create(GroupboxContainer, Library.TweenInfo, {
                        Position = UDim2.fromOffset(0, 35),
                    ...
                    Groupbox:Resize()
                    TweenService:Create(GroupboxContainer, Library.TweenInfo, {
                        Size = UDim2.new(1, 0, 0, GroupboxList.AbsoluteContentSize.Y / Library.DPIScale + 14),
                    }):Play()
                end
            end
