...
do
    ...
    function Funcs:AddLabel(...)
        ...
        -- Icon support for label
        if Data.Icon and not Label.DoesWrap then
            local Img = AttachElementIcon(TextLabel, Data.Icon, 14)
            if Img then
    ...
    function Funcs:AddButton(...)
        ...
        local function CreateButton(Button)
            ...
            if Button.Icon then
                local Img = AttachElementIcon(BtnContent, Button.Icon, 13)
                if Img then
                    Img.AnchorPoint = Vector2.new(0, 0)
                    Img.Position = UDim2.fromOffset(0, 0)
                    Img.Size = UDim2.fromOffset(13, 13)
                    Img.ImageColor3 = Button.Risky and Library.Scheme.RedColor or Library.Scheme.AccentColor
                    Library.Registry[Img] = { ImageColor3 = Button.Risky and "RedColor" or "AccentColor" }
                end
            end
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        if Info.Icon then
            local Img = AttachElementIcon(Button, Info.Icon, 14, 22)
            if Img then
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        if Info.Icon then
            local Img = AttachElementIcon(Button, Info.Icon, 14, 0)
            if Img then
                ToggleIconOffset = 18
                Img.AnchorPoint = Vector2.new(0, 0.5)
                Img.Position = UDim2.new(0, 0, 0.5, 0)
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        if Info.Icon then
            local Img = AttachElementIcon(Holder, Info.Icon, 12, 0, -10)
            if Img then
                Img.AnchorPoint = Vector2.new(0, 0)
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        if not Info.Compact then
            ...
            if Info.Icon then
                local Img = AttachElementIcon(Holder, Info.Icon, 12, 0, 0)
                if Img then
                    Img.AnchorPoint = Vector2.new(0, 0)
                    Img.Position = UDim2.fromOffset(0, 1)
