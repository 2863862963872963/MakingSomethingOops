...
-- Sets overall UI background transparency (0 = opaque, 1 = fully transparent).
-- Only affects solid background frames — text, icons and strokes are unaffected.
function Library:SetUITransparency(Alpha: number)
    Alpha = math.clamp(Alpha, 0, 1)
    Library.UITransparency = Alpha

    for Instance, _ in Library.Registry do
        local ok, current = pcall(function() return Instance.BackgroundTransparency end)
        if ok and current ~= nil then
            -- only touch instances that have an opaque background registered in the scheme
            local Props = Library.Registry[Instance]
