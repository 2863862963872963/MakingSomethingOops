...
local Library = {
    ...
    --// Style Config \\--
    Style = {
        AccentStrip           = true,   -- Left-edge colored strip on groupboxes
        HeaderGradient        = true,   -- Subtle gradient on groupbox headers
        ElementIconSize       = 16,     -- px size for element row icons
        ElementHeight         = nil,    -- Override row height (nil = use defaults)
        HeaderHeight          = 34,     -- Groupbox header height in px
        AnimationSpeed        = 0.1,    -- TweenInfo time override
        CollapseAnimation     = true,   -- Animate section collapse
        WindowTransparency    = 0,      -- 0 = fully opaque, 1 = fully transparent
        GroupboxTransparency  = 0,      -- 0 = fully opaque, 1 = fully transparent
    },

    GroupboxHolders = {},  -- tracked for live transparency changes
    MainFrame = nil,       -- set by CreateWindow for live transparency changes
}
...
Library.Themes = {
    ...
    Light = {
        BackgroundColor = Color3.fromRGB(235, 235, 240),
        MainColor       = Color3.fromRGB(255, 255, 255),
        AccentColor     = Color3.fromRGB(80, 60, 200),
        OutlineColor    = Color3.fromRGB(200, 200, 210),
        FontColor       = Color3.fromRGB(15, 15, 20),
        IsLight         = true,
    },
    Ocean = {
        BackgroundColor = Color3.fromRGB(10, 20, 35),
        MainColor       = Color3.fromRGB(15, 30, 52),
        AccentColor     = Color3.fromRGB(50, 150, 255),
        OutlineColor    = Color3.fromRGB(28, 55, 85),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Ember = {
        BackgroundColor = Color3.fromRGB(20, 10, 8),
        MainColor       = Color3.fromRGB(35, 18, 14),
        AccentColor     = Color3.fromRGB(255, 95, 40),
        OutlineColor    = Color3.fromRGB(65, 30, 22),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Midnight = {
        BackgroundColor = Color3.fromRGB(12, 10, 22),
        MainColor       = Color3.fromRGB(22, 18, 38),
        AccentColor     = Color3.fromRGB(170, 100, 255),
        OutlineColor    = Color3.fromRGB(48, 38, 70),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
    Nord = {
        BackgroundColor = Color3.fromRGB(36, 41, 51),
        MainColor       = Color3.fromRGB(46, 52, 64),
        AccentColor     = Color3.fromRGB(129, 161, 193),
        OutlineColor    = Color3.fromRGB(67, 76, 94),
        FontColor       = Color3.fromRGB(236, 239, 244),
        IsLight         = false,
    },
    Rose = {
        BackgroundColor = Color3.fromRGB(20, 14, 18),
        MainColor       = Color3.fromRGB(35, 24, 30),
        AccentColor     = Color3.fromRGB(255, 100, 160),
        OutlineColor    = Color3.fromRGB(65, 40, 55),
        FontColor       = Color3.new(1, 1, 1),
        IsLight         = false,
    },
}

function Library:SetStyle(StyleOverrides)
    assert(typeof(StyleOverrides) == "table", "StyleOverrides must be a table")
    for Key, Value in StyleOverrides do
        Library.Style[Key] = Value
    end

    if StyleOverrides.AnimationSpeed ~= nil then
        Library.TweenInfo = TweenInfo.new(
            Library.Style.AnimationSpeed,
            Enum.EasingStyle.Quad,
            Enum.EasingDirection.Out
        )
    end

    if StyleOverrides.WindowTransparency ~= nil and Library.MainFrame then
        TweenService:Create(Library.MainFrame, Library.TweenInfo, {
            BackgroundTransparency = Library.Style.WindowTransparency,
        }):Play()
    end

    if StyleOverrides.GroupboxTransparency ~= nil then
        for _, Holder in Library.GroupboxHolders do
            if Holder and Holder.Parent then
                TweenService:Create(Holder, Library.TweenInfo, {
                    BackgroundTransparency = Library.Style.GroupboxTransparency,
                }):Play()
            end
        end
    end
...
function Library:SetTheme(ThemeName)
    ...
    assert(Theme, string.format("Theme %q does not exist. Available: Dark, Light, Ocean, Ember, Midnight, Nord, Rose", ThemeName))
...
function Library:AddTheme(ThemeName, ThemeData)
    assert(typeof(ThemeName) == "string", "ThemeName must be a string")
    assert(typeof(ThemeData) == "table", "ThemeData must be a table")
...
do
    ...
    function Funcs:AddFadeDivider(TextOrInfo)
        local Text
        local MarginTop = 0
        local MarginBottom = 0

        if typeof(TextOrInfo) == "table" then
            Text = TextOrInfo.Text
            MarginTop = TextOrInfo.MarginTop or TextOrInfo.Margin or 0
            MarginBottom = TextOrInfo.MarginBottom or TextOrInfo.Margin or 0
        elseif typeof(TextOrInfo) == "string" then
            Text = TextOrInfo
        end
        ...
        local TotalHeight = (Text and 18 or 6) + MarginTop + MarginBottom
        ...
        local function MakeFadeLine(AnchorX, PosX, SizeXScale, SizeXOffset, GradientRotation)
            local Line = New("Frame", {
                AnchorPoint = Vector2.new(AnchorX, 0.5),
                BackgroundColor3 = "OutlineColor",
                BorderSizePixel = 0,
                Position = UDim2.new(PosX, 0, 0.5, 0),
                Size = UDim2.new(SizeXScale, SizeXOffset, 0, 1),
                Parent = InnerHolder,
            })
            New("UIGradient", {
                Transparency = NumberSequence.new({
        ...
        if Text and Text ~= "" then
            ...
            local TextLabel_AbsX = 0
            TextLabel:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
                TextLabel_AbsX = TextLabel.AbsoluteSize.X / 2
            end)

            MakeFadeLine(1, 0.5, 0.5, -8, 180)
            MakeFadeLine(0, 0.5,  0.5, -8, 0)
        else
            local Line = New("Frame", {
                AnchorPoint = Vector2.new(0.5, 0.5),
                BackgroundColor3 = "OutlineColor",
    ...
    function Funcs:AddButton(...)
        ...
        local Holder = New("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, Library.Style.ElementHeight or 21),
            Parent = Container,
        })
        ...
        local function CreateButton(Button)
            ...
            --// Optional icon overlay inside button
            if Info.Icon then
                local BtnIcon = Library:GetCustomIcon(Info.Icon)
                if BtnIcon then
                    local IconSize = Library.Style.ElementIconSize or 16
                    New("ImageLabel", {
                        Image = BtnIcon.Url,
                        ImageColor3 = BtnIcon.Custom and "WhiteColor" or "AccentColor",
                    ...
                    New("UIPadding", {
                        PaddingLeft = UDim.new(0, IconSize + 10),
                        Parent = Base,
                    })
                end
            end
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        local Button = New("TextButton", {
            ...
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, Library.Style.ElementHeight or 18),
            Text = "",
            Visible = Toggle.Visible,
            Parent = Container,
        })

        --// Optional row icon (before checkbox)
        local CheckboxIconOffset = 0
        if Info.Icon then
            local ElemIcon = Library:GetCustomIcon(Info.Icon)
            if ElemIcon then
                local IconSize = Library.Style.ElementIconSize or 16
                CheckboxIconOffset = IconSize + 4
                New("ImageLabel", {
                    Image = ElemIcon.Url,
                    ImageColor3 = ElemIcon.Custom and "WhiteColor" or "AccentColor",
                    ImageRectOffset = ElemIcon.ImageRectOffset,
                    ImageRectSize = ElemIcon.ImageRectSize,
                    ImageTransparency = 0.2,
                    AnchorPoint = Vector2.new(0, 0.5),
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        --// Optional row icon left of label
        local InputIconOffset = 0
        if Info.Icon then
            local ElemIcon = Library:GetCustomIcon(Info.Icon)
            if ElemIcon then
                local IconSize = Library.Style.ElementIconSize or 16
                InputIconOffset = IconSize + 4
                New("ImageLabel", {
                    Image = ElemIcon.Url,
                    ImageColor3 = ElemIcon.Custom and "WhiteColor" or "AccentColor",
                    ImageRectOffset = ElemIcon.ImageRectOffset,
        ...
        local Label = New("TextLabel", {
            BackgroundTransparency = 1,
            Position = UDim2.fromOffset(InputIconOffset, 0),
            Size = UDim2.new(1, -InputIconOffset, 0, 14),
            Text = Input.Text,
            TextSize = 14,
            TextXAlignment = Enum.TextXAlignment.Left,
            Parent = Holder,
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        if not Info.Compact then
            --// Optional row icon
            local SliderIconOffset = 0
            if Info.Icon then
                local ElemIcon = Library:GetCustomIcon(Info.Icon)
                if ElemIcon then
                    local IconSize = Library.Style.ElementIconSize or 16
                    SliderIconOffset = IconSize + 4
                    New("ImageLabel", {
                        Image = ElemIcon.Url,
                        ImageColor3 = ElemIcon.Custom and "WhiteColor" or "AccentColor",
                        ImageRectOffset = ElemIcon.ImageRectOffset,
            ...
            SliderLabel = New("TextLabel", {
                BackgroundTransparency = 1,
                Position = UDim2.fromOffset(SliderIconOffset, 0),
                Size = UDim2.new(1, -SliderIconOffset, 0, 14),
                Text = Slider.Text,
                TextSize = 14,
                TextXAlignment = Enum.TextXAlignment.Left,
                Parent = Holder,
...
function Library:Notify(...)
    ...
    if typeof(Info) == "table" then
        ...
        Data.ClickDismiss = Info.ClickDismiss ~= false  -- default true
        Data.Style = Info.Style  -- nil | "Progress"
        if Data.Style == "Progress" then
            Data.Persist = true  -- progress notifs are always persistent until done
        end
    else
        Data.Description = tostring(Info)
        Data.Time = select(2, ...) or 5
        Data.SoundId = select(3, ...)
        Data.ClickDismiss = true
    end
    ...
    local IsProgressStyle = Data.Style == "Progress"
    local TimerBarHeight = IsProgressStyle and 4 or 2
    local TimerBarOffset = IsProgressStyle and 1 or 3

    local TimerHolder = New("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 0, IsProgressStyle and 8 or 7),
        Visible = (Data.Persist ~= true and typeof(Data.Time) ~= "Instance") or typeof(Data.Steps) == "number" or IsProgressStyle,
        Parent = Holder,
    })
    local TimerBar = New("Frame", {
        ...
        BorderColor3 = "OutlineColor",
        BorderSizePixel = 1,
        Position = UDim2.fromOffset(0, TimerBarOffset),
        Size = UDim2.new(1, 0, 0, TimerBarHeight),
        Parent = TimerHolder,
    })
    if IsProgressStyle then
        New("UICorner", {
            CornerRadius = UDim.new(1, 0),
            Parent = TimerBar,
        })
    ...
    TimerFill = New("Frame", {
        BackgroundColor3 = "AccentColor",
        Size = IsProgressStyle and UDim2.fromScale(0, 1) or UDim2.fromScale(1, 1),
        Parent = TimerBar,
    })
    if IsProgressStyle then
        New("UICorner", {
    ...
    function Data:SetProgress(Value: number)
        Value = math.clamp(Value or 0, 0, 1)
        TweenService:Create(TimerFill, TweenInfo.new(Library.TweenInfo.Time, Enum.EasingStyle.Quad), {
            Size = UDim2.fromScale(Value, 1),
        }):Play()
        if Value >= 1 then
            task.delay(Library.TweenInfo.Time + 0.05, function()
    ...
    if Data.ClickDismiss then
        ...
        Holder.MouseEnter:Connect(function()
            TweenService:Create(Holder, Library.TweenInfo, {
                BackgroundColor3 = Library:GetBetterColor(Library.Scheme.MainColor, 6),
            }):Play()
        end)
...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddGroupbox(Info)
            ...
            local HeaderH = Library.Style.HeaderHeight or 34
            ...
            do
                ...
                if Library.Style.AccentStrip then
                    local Strip = New("Frame", {
                        BackgroundColor3 = "AccentColor",
                        Position = UDim2.fromOffset(0, 0),
                        Size = UDim2.new(0, 3, 0, HeaderH),
                    ...
                    --// Cover the right corners so only left side rounds
                    New("Frame", {
                        BackgroundColor3 = "AccentColor",
                        AnchorPoint = Vector2.new(1, 0),
                        Position = UDim2.fromScale(1, 0),
                ...
                --// Header gradient
                if Library.Style.HeaderGradient then
                    local HeaderGradientFrame = New("Frame", {
                        BackgroundColor3 = "AccentColor",
                        BackgroundTransparency = 0.88,
                        Size = UDim2.new(1, 0, 0, HeaderH),
                        ZIndex = GroupboxHolder.ZIndex,
                        Parent = GroupboxHolder,
                    })
                    New("UICorner", {
                        CornerRadius = UDim.new(0, WindowInfo.CornerRadius),
                    ...
                    --// Cover bottom half so gradient only sits on the header
                    New("Frame", {
                        AnchorPoint = Vector2.new(0, 1),
                        BackgroundColor3 = "BackgroundColor",
                        Position = UDim2.fromScale(0, 1),
                        Size = UDim2.new(1, 0, 1, 0),
                        ZIndex = HeaderGradientFrame.ZIndex,
                        Parent = HeaderGradientFrame,
                    })
                end

                HeaderDivider = Library:MakeLine(GroupboxHolder, {
                ...
                if BoxIcon then
                    New("ImageLabel", {
                        ...
                        Position = UDim2.fromOffset(Library.Style.AccentStrip and 10 or 6, 6),
                        Size = UDim2.fromOffset(22, 22),
                        ZIndex = GroupboxHolder.ZIndex + 2,
                        Parent = GroupboxHolder,
                    })
                end

                local LabelXOffset = (Library.Style.AccentStrip and 8 or 0) + (BoxIcon and 24 or 0)
                GroupboxLabel = New("TextLabel", {
                    BackgroundTransparency = 1,
                    Position = UDim2.fromOffset(LabelXOffset, 0),
                    Size = UDim2.new(1, -LabelXOffset - 28, 0, HeaderH),
                    Text = Info.Name,
                    TextSize = 15,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    ZIndex = GroupboxHolder.ZIndex + 2,
                ...
                local ArrowBtn = New("TextButton", {
                    AnchorPoint = Vector2.new(1, 0.5),
                    BackgroundTransparency = 1,
                    Position = UDim2.new(1, -8, 0, HeaderH / 2),
                    Size = UDim2.fromOffset(18, 18),
                    Text = "",
                    ZIndex = GroupboxHolder.ZIndex + 3,
                    Parent = GroupboxHolder,
                })
                CollapseButton = ArrowBtn
                ...
                GroupboxContainer = New("Frame", {
                    BackgroundTransparency = 1,
                    Position = UDim2.fromOffset(0, HeaderH + 1),
            ...
            function Groupbox:Resize()
                if Groupbox.Collapsed then
                    GroupboxHolder.Size = UDim2.new(1, 0, 0, HeaderH + 1)
                else
                    GroupboxHolder.Size = UDim2.new(1, 0, 0, (GroupboxList.AbsoluteContentSize.Y / Library.DPIScale) + HeaderH + 15)
                end
            end

            function Groupbox:Collapse()
                Groupbox.Collapsed = true
                local ArrowImg = CollapseButton:FindFirstChildOfClass("ImageLabel") or CollapseButton:FindFirstChildOfClass("TextLabel")

                GroupboxContainer.Visible = false
                HeaderDivider.Visible = false

                if Library.Style.CollapseAnimation then
                    TweenService:Create(ArrowImg, Library.TweenInfo, {
                        Rotation = 180,
                    }):Play()
                else
                    if ArrowImg then ArrowImg.Rotation = 180 end
                end
                Groupbox:Resize()
            end
            ...
            function Groupbox:Expand()
                ...
                if Library.Style.CollapseAnimation then
                    ...
                    if ArrowImg then ArrowImg.Rotation = 0 end
                end
                Groupbox:Resize()
            end
            ...
            --// Wire collapse button click
            CollapseButton.MouseButton1Click:Connect(function()
                Groupbox:ToggleCollapse()
            end)

            --// Auto-collapse if Info.Collapsed = true
            if Info.Collapsed then
                task.defer(function() Groupbox:Collapse() end)
            end
        ...
        function Tab:AddLeftGroupbox(Name, IconName, ExtraInfo)
        ...
        function Tab:AddRightGroupbox(Name, IconName, ExtraInfo)
            local Info = ExtraInfo or {}
            Info.Side = 2
            Info.Name = Name
            Info.IconName = IconName
        ...
        function Tab:AddSettingPage()
            local LeftAppearance  = Tab:AddLeftGroupbox("Appearance",    "palette")
            local RightVisual     = Tab:AddRightGroupbox("Visual Style", "layers")
            local LeftWindow      = Tab:AddLeftGroupbox("Window",        "monitor")
            local RightBehavior   = Tab:AddRightGroupbox("Behavior",     "sliders")

            --// APPEARANCE \\--
            local ThemeNames = {}
            for K in Library.Themes do table.insert(ThemeNames, K) end
            table.sort(ThemeNames)

            LeftAppearance:AddDropdown("_Settings_Theme", {
                Text    = "Theme",
                Values  = ThemeNames,
                Default = "Dark",
            ...
            LeftAppearance:AddColorPicker("_Settings_AccentColor", {
                Text    = "Accent Color",
            ...
            LeftAppearance:AddColorPicker("_Settings_FontColor", {
                Text    = "Font Color",
            ...
            LeftAppearance:AddDropdown("_Settings_Font", {
                Text    = "Font",
                Values  = FontNames,
                Default = "Code",
                Callback = function(Value)
            ...
            --// VISUAL STYLE \\--
            RightVisual:AddToggle("_Settings_AccentStrip", {
                Text    = "Accent Strip",
                Icon    = "sidebar",
                Default = Library.Style.AccentStrip,
                Callback = function(Value)
                    Library:SetStyle({ AccentStrip = Value })
                end,
            })

            RightVisual:AddToggle("_Settings_HeaderGradient", {
                Text    = "Header Gradient",
                Icon    = "blend",
                Default = Library.Style.HeaderGradient,
                Callback = function(Value)
                    Library:SetStyle({ HeaderGradient = Value })
                end,
            })

            RightVisual:AddToggle("_Settings_CollapseAnim", {
                Text    = "Collapse Animation",
                Icon    = "chevrons-down",
                Default = Library.Style.CollapseAnimation,
                Callback = function(Value)
                    Library:SetStyle({ CollapseAnimation = Value })
                end,
            })

            RightVisual:AddSlider("_Settings_ElemIconSize", {
                Text     = "Icon Size",
                Icon     = "image",
                Default  = Library.Style.ElementIconSize,
                Min      = 10,
                Max      = 24,
                Rounding = 0,
                Suffix   = "px",
                Callback = function(Value)
                    Library:SetStyle({ ElementIconSize = Value })
                end,
            })

            RightVisual:AddSlider("_Settings_AnimSpeed", {
                Text     = "Animation Speed",
                Icon     = "timer",
                Default  = math.floor(Library.Style.AnimationSpeed * 100),
                Min      = 1,
                Max      = 60,
                Rounding = 0,
                Callback = function(Value)
                    Library:SetStyle({ AnimationSpeed = Value / 100 })
                end,
            })

            --// WINDOW \\--
            LeftWindow:AddSlider("_Settings_WindowOpacity", {
                Text     = "Window Opacity",
                Icon     = "layout-dashboard",
                Default  = math.floor((1 - Library.Style.WindowTransparency) * 100),
                Min      = 20,
                Max      = 100,
                Rounding = 0,
                Suffix   = "%",
                Callback = function(Value)
                    Library:SetStyle({ WindowTransparency = 1 - Value / 100 })
                end,
            })

            LeftWindow:AddSlider("_Settings_GroupboxOpacity", {
                Text     = "Groupbox Opacity",
                Icon     = "square",
                Default  = math.floor((1 - Library.Style.GroupboxTransparency) * 100),
                Min      = 0,
                Max      = 100,
                Rounding = 0,
                Suffix   = "%",
                Callback = function(Value)
                    Library:SetStyle({ GroupboxTransparency = 1 - Value / 100 })
                end,
            })

            LeftWindow:AddSlider("_Settings_CornerRadius", {
                Text     = "Corner Radius",
                Icon     = "rounded-corner",
            ...
            LeftWindow:AddDropdown("_Settings_NotifySide", {
                Text    = "Notify Side",
                Icon    = "bell",
                Values  = { "Left", "Right" },
            ...
            --// BEHAVIOR \\--
            RightBehavior:AddToggle("_Settings_CustomCursor", {
                Text    = "Custom Cursor",
            ...
            RightBehavior:AddToggle("_Settings_ForceCheckbox", {
                Text    = "Force Checkbox Style",
                Icon    = "check-square",
            ...
            RightBehavior:AddToggle("_Settings_NotifyOnError", {
            ...
            RightBehavior:AddFadeDivider("DPI")

            RightBehavior:AddSlider("_Settings_DPIScale", {
                Text     = "DPI Scale",
                Icon     = "zoom-in",
            ...
            --// CONFIG PROFILES \\--
            local LeftConfig = Tab:AddLeftGroupbox("Config Profiles", "save")

            local ProfileInput = { Value = "default" }

            LeftConfig:AddInput("_Settings_ProfileName", {
                Text        = "Profile Name",
                Icon        = "tag",
                Default     = "default",
                Placeholder = "profile name",
                Callback = function(Value)
                    ProfileInput.Value = Value ~= "" and Value or "default"
                end,
            })

            LeftConfig:AddFadeDivider()

            LeftConfig:AddButton({
                Text = "Save Config",
                Icon = "save",
                Func = function()
                    local ok = Library:SaveConfig(ProfileInput.Value)
                    Library:Notify({
                        Title = ok and "Config Saved" or "Save Failed",
                        Description = ok and ("Saved as: " .. ProfileInput.Value) or "Could not write file.",
            ...
            LeftConfig:AddButton({
                ...
                Icon = "folder-open",
                Func = function()
                    local ok = Library:LoadConfig(ProfileInput.Value)
                    Library:Notify({
                        Title = ok and "Config Loaded" or "Load Failed",
                        Description = ok and ("Loaded: " .. ProfileInput.Value) or "Profile not found.",
                        Icon = ok and "check-circle" or "alert-circle",
                        Time = 3,
                    })
                end,
            ...
            LeftConfig:AddButton({
                ...
                Risky = true,
                Func  = function()
                    local ok = Library:DeleteProfile(ProfileInput.Value)
                    Library:Notify({
                        Title = ok and "Profile Deleted" or "Delete Failed",
                        Description = ok and ("Deleted: " .. ProfileInput.Value) or "Profile not found.",
                        Icon = ok and "trash-2" or "alert-circle",
                        Time = 3,
                    })
                end,
            })

            local RightConfig = Tab:AddRightGroupbox("Saved Profiles", "list")

            local function RefreshProfileList()
                local Profiles = Library:GetProfiles()
                if #Profiles == 0 then
            ...
            RightConfig:AddDropdown("_Settings_ProfileList", {
                Text   = "Profiles",
                Icon   = "files",
                Values = RefreshProfileList(),
                Callback = function(Value)
                    if Value ~= "No profiles found" then
                        ProfileInput.Value = Value
                        if Library.Options["_Settings_ProfileName"] then
                            Library.Options["_Settings_ProfileName"]:SetValue(Value)
                        end
                    end
                end,
            })

            RightConfig:AddButton({
                Text = "Refresh List",
                Icon = "refresh-cw",
                Func = function()
                    if Library.Options["_Settings_ProfileList"] then
                        Library.Options["_Settings_ProfileList"]:SetValues(RefreshProfileList())
                    end
                end,
            })
        end
    ...
    if Library.IsMobile or WindowInfo.ToggleUiVariant == 2 then
        local BtnInfo = WindowInfo.ToggleUiButton or {}
        local Shape      = BtnInfo.Shape      or "Round"
        local BtnSize    = BtnInfo.Size        or UDim2.fromOffset(40, 40)
        local BtnColor   = BtnInfo.Color       or nil
        local BtnPos     = BtnInfo.Position    or UDim2.fromOffset(6, 6)
        local BtnSide    = BtnInfo.Side        or "Left"
        local OpenIcon   = BtnInfo.OpenIcon    or "x"
        local ClosedIcon = BtnInfo.ClosedIcon  or "menu"
        local ShowLock   = BtnInfo.ShowLock
        if ShowLock == nil then ShowLock = true end

        local CornerScale = Shape == "Circle" and 1 or (Shape == "Square" and 0 or 0.25)
        local BtnW = BtnSize.X.Offset > 0 and BtnSize.X.Offset or 40
        local BtnH = BtnSize.Y.Offset > 0 and BtnSize.Y.Offset or 40
        local IconSz = math.floor(math.min(BtnW, BtnH) * 0.5)

        if WindowInfo.ToggleUiVariant == 2 then
            --// Variant 2 — custom icon button \\--
            local OpenIconData   = Library:GetCustomIcon(OpenIcon)
            local ClosedIconData = Library:GetCustomIcon(ClosedIcon)

            local Btn = New("TextButton", {
                AnchorPoint = BtnSide == "Right" and Vector2.new(1, 0) or Vector2.zero,
                BackgroundColor3 = BtnColor or Library.Scheme.AccentColor,
                Position = BtnPos,
                Size = BtnSize,
                Text = "",
                ZIndex = 10,
            ...
            table.insert(Library.Scales, New("UIScale", { Parent = Btn }))
            ...
            local function UpdateToggleIcon()
                local IconData = Library.Toggled and OpenIconData or ClosedIconData
                if IconData then
            ...
            --// Lock button (optional, sits below)
            if ShowLock then
                local LockBtn = New("TextButton", {
                    ...
                    Position = UDim2.new(
                        ...
                        BtnPos.Y.Scale, BtnPos.Y.Offset + BtnH + 6
                    ),
                    Size = BtnSize,
                    Text = "",
                    ZIndex = 10,
                ...
                table.insert(Library.Scales, New("UIScale", { Parent = LockBtn }))
                Library:AddOutline(LockBtn)
                Library:MakeDraggable(LockBtn, LockBtn, true)

                local LockIcon = Library:GetCustomIcon("lock")
                local UnlockIcon = Library:GetCustomIcon("lock-open")
                local LockImg = New("ImageLabel", {
                    AnchorPoint = Vector2.new(0.5, 0.5),
                    BackgroundTransparency = 1,
                    Position = UDim2.fromScale(0.5, 0.5),
                    Size = UDim2.fromOffset(IconSz, IconSz),
                    ZIndex = LockBtn.ZIndex + 1,
                    Parent = LockBtn,
                })

                local function UpdateLockIcon()
                    local IconData = Library.CantDragForced and LockIcon or UnlockIcon
                    if IconData then
                        LockImg.Image = IconData.Url
                        LockImg.ImageRectOffset = IconData.ImageRectOffset
                        LockImg.ImageRectSize = IconData.ImageRectSize
                        LockImg.ImageColor3 = Library.Scheme.FontColor
                    end
                end
                UpdateLockIcon()
            ...
            --// Variant 1 — original text buttons (mobile default) \\--
...
--// Config System \\--
do
    local CONFIG_DIR = "Obsidian/profiles"
    ...
    local function SerializeColor(c: Color3)
    ...
    function Library:SaveConfig(ProfileName: string)
        ProfileName = ProfileName or "default"
        assert(typeof(ProfileName) == "string" and #ProfileName > 0, "ProfileName must be a non-empty string")

        if not EnsureDir() then
            warn("[Obsidian Config] writefile/makefolder not available — cannot save.")
            return false
        end
        ...
        for Idx, Toggle in Library.Toggles do
            if Toggle.Type == "Toggle" then
                Data.toggles[tostring(Idx)] = Toggle.Value
            end
        end

        for Idx, Option in Library.Options do
            local Entry = { type = Option.Type }
            if Option.Type == "Slider" or Option.Type == "Input" then
                Entry.value = Option.Value
            elseif Option.Type == "Dropdown" then
                if typeof(Option.Value) == "table" then
                    local multi = {}
                    for k, v in Option.Value do multi[k] = v end
                    Entry.value = multi
                    Entry.multi = true
                else
                    Entry.value = Option.Value
            ...
            elseif Option.Type == "ColorPicker" then
                Entry.value = SerializeColor(Option.Value)
            end
        ...
        if not ok then
            warn("[Obsidian Config] Failed to encode config:", encoded)
            return false
        end

        local written, err = pcall(writefile, ProfilePath(ProfileName), encoded)
        if not written then
            warn("[Obsidian Config] Failed to write file:", err)
            return false
        end
    ...
    function Library:LoadConfig(ProfileName: string)
        ProfileName = ProfileName or "default"
        assert(typeof(ProfileName) == "string" and #ProfileName > 0, "ProfileName must be a non-empty string")

        if not readfile or not isfile then
            warn("[Obsidian Config] readfile/isfile not available — cannot load.")
            return false
        end

        local path = ProfilePath(ProfileName)
        if not isfile(path) then
            warn("[Obsidian Config] Profile not found:", ProfileName)
            return false
        end

        local ok, raw = pcall(readfile, path)
        if not ok then
            warn("[Obsidian Config] Failed to read file:", raw)
            return false
        end

        local decoded, Data
        ok, Data = pcall(game:GetService("HttpService").JSONDecode, game:GetService("HttpService"), raw)
        if not ok then
            warn("[Obsidian Config] Failed to decode config:", Data)
            return false
        end

        if Data.toggles then
            for Idx, Value in Data.toggles do
                local Toggle = Library.Toggles[Idx] or Library.Toggles[tonumber(Idx)]
                if Toggle and typeof(Value) == "boolean" then
        ...
        if Data.options then
            for Idx, Entry in Data.options do
                local Option = Library.Options[Idx] or Library.Options[tonumber(Idx)]
                if not Option then continue end

                if Entry.type == "Slider" and typeof(Entry.value) == "number" then
                    Option:SetValue(Entry.value)
                elseif Entry.type == "Input" and typeof(Entry.value) == "string" then
                    Option:SetValue(Entry.value)
                elseif Entry.type == "Dropdown" then
                    if Entry.multi and typeof(Entry.value) == "table" then
                        Option:SetValue(Entry.value)
                    elseif typeof(Entry.value) == "string" then
                        Option:SetValue(Entry.value)
                    end
                elseif Entry.type == "ColorPicker" and typeof(Entry.value) == "table" then
                    Option:SetValue(DeserializeColor(Entry.value))
                end
            end
        end
    ...
    function Library:DeleteProfile(ProfileName: string)
        assert(typeof(ProfileName) == "string" and #ProfileName > 0, "ProfileName must be a non-empty string")

        if not delfile or not isfile then
            warn("[Obsidian Config] delfile not available — cannot delete.")
            return false
        end
        ...
        if not ok then
            warn("[Obsidian Config] Failed to delete profile:", err)
            return false
        end
