...
do
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        --// Drag Value Tooltip \\--
        local DragTooltip = New("TextLabel", {
            AutomaticSize = Enum.AutomaticSize.X,
            BackgroundColor3 = "BackgroundColor",
        ...
        New("UIStroke", { Color = "OutlineColor", Parent = DragTooltip })
        table.insert(Library.Corners, New("UICorner", {
            CornerRadius = UDim.new(0, Library.CornerRadius / 2),
            Parent = DragTooltip,
        }))
        table.insert(Library.Scales, New("UIScale", { Parent = DragTooltip }))
        ...
        Bar.InputBegan:Connect(function(Input: InputObject)
            ...
            while IsDragInput(Input) do
                ...
                --// Update drag tooltip position and text \\--
                DragTooltip.Text = DisplayLabel.Text
                DragTooltip.Visible = true
                local FillRight = Bar.AbsolutePosition.X + Fill.AbsoluteSize.X
                local TooltipW  = DragTooltip.AbsoluteSize.X
                local TooltipX  = math.clamp(FillRight - TooltipW / 2, 4, workspace.CurrentCamera.ViewportSize.X - TooltipW - 4)
                local TooltipY  = Bar.AbsolutePosition.Y - 24
                DragTooltip.Position = UDim2.fromOffset(TooltipX, TooltipY)

                RunService.RenderStepped:Wait()
            end

            --// Hide drag tooltip after release \\--
...
function Library:Notify(...)
    ...
    if typeof(Info) == "table" then
        ...
        Data.Type = Info.Type          -- "success" | "error" | "warning" | "info"
    ...
    -- Sets Icon and IconColor automatically unless the caller overrides them.
    local NotifyTypePresets = {
        success = { Icon = "check-circle",   IconColor = Color3.fromRGB(50,  200, 80)  },
        error   = { Icon = "x-circle",       IconColor = Color3.fromRGB(220, 50,  50)  },
        warning = { Icon = "alert-triangle", IconColor = Color3.fromRGB(255, 165, 0)   },
        info    = { Icon = "info",           IconColor = Color3.fromRGB(80,  150, 255) },
    }
    if Data.Type then
        local Preset = NotifyTypePresets[tostring(Data.Type):lower()]
        if Preset then
            if not Data.Icon and not Data.BigIcon then
                Data.Icon = Preset.Icon
            end
            if not Data.IconColor then
                Data.IconColor = Preset.IconColor
            end
            Data.AccentColor = Data.AccentColor or Preset.IconColor
        end
    end
    ...
    --// Type Accent Bar (left-edge colored stripe) \\--
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                local CollapseChevronIcon = Library:GetIcon("chevron-up")
            ...
            function Groupbox:Resize()
                if Groupbox.Collapsed then return end
                GroupboxHolder.Size = UDim2.new(1, 0, 0, (GroupboxList.AbsoluteContentSize.Y / Library.DPIScale) + 49)
            end

            --// Collapse / Expand \\--
            function Groupbox:SetCollapsed(Value: boolean)
                Groupbox.Collapsed = Value

                if Groupbox.Collapsed then
                    --// Collapse: shrink to header height only
                    ...
                    TweenService:Create(CollapseBtn, Library.TweenInfo, {
                        Rotation = 180,
                    }):Play()
                else
                    --// Expand: restore full content height
                    GroupboxHolder.ClipsDescendants = false
                    GroupboxContainer.Visible = true
                    local FullHeight = (GroupboxList.AbsoluteContentSize.Y / Library.DPIScale) + 49
                    TweenService:Create(GroupboxHolder, Library.TweenInfo, {
            ...
            GroupboxLabel.MouseEnter:Connect(function()
                TweenService:Create(CollapseBtn, Library.TweenInfo, {
                    BackgroundTransparency = 0.85,
                }):Play()
            end)
            GroupboxLabel.MouseLeave:Connect(function()
                TweenService:Create(CollapseBtn, Library.TweenInfo, {
                    BackgroundTransparency = 1,
                }):Play()
            end)
