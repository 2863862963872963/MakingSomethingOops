...
-- Transparency roles — each key maps to a BackgroundColor3 scheme key.
-- The value is a multiplier (0–1) applied to the user's Alpha so that
-- different layers dim uniformly rather than all going to the same flat value.
local TransparencyRoles = {
    ...
    OutlineColor    = 0.00,  -- borders never transparent
}
...
local function ConfigFilePath(Name)
    return Library.ConfigPath .. "/" .. Name .. ".json"
end

-- Serialise all Toggle + Option values to a flat table
local function SerialiseState()
    local Data = {}
    for Idx, Toggle in Library.Toggles do
        Data[tostring(Idx)] = Toggle.Value
    end
    for Idx, Option in Library.Options do
        ...
        elseif typeof(v) == "table" then
            -- multi-dropdown: shallow copy
            local t = {}
            for k, val in v do t[tostring(k)] = val end
            v = t
        end
        Data[tostring(Idx)] = v
    end
...
-- Apply a flat table back onto Toggles + Options
local function ApplyState(Data)
    for Idx, Toggle in Library.Toggles do
        local v = Data[tostring(Idx)]
    ...
    for Idx, Option in Library.Options do
        ...
        pcall(function()
            ...
            elseif typeof(v) == "table" then
                -- multi-dropdown: convert string keys back
                local restored = {}
                for k, val in v do restored[k] = val end
                Option:SetValue(restored)
            else
                Option:SetValue(v)
            end
        end)
...
do
    -- lightweight pure-Lua JSON encoder/decoder
    local function Encode(val, indent, level)
        indent = indent or ""
        level  = level  or 0
        local T = typeof(val)
        ...
        elseif T == "table"   then
            ...
            for k in val do
                n += 1
                if typeof(k) ~= "number" or k ~= math.floor(k) or k < 1 then isArr = false break end
            end
            if isArr and n == #val then
                local parts = {}
                for _, v in ipairs(val) do table.insert(parts, Encode(v, indent, level+1)) end
                return "[" .. table.concat(parts, ",") .. "]"
            else
                local parts = {}
                for k, v in val do
                    table.insert(parts, Encode(tostring(k)) .. ":" .. Encode(v, indent, level+1))
                end
                return "{" .. table.concat(parts, ",") .. "}"
            end
        end
    ...
    local function Decode(s)
        local pos = 1
        local function skip() while pos <= #s and s:sub(pos,pos):match("%s") do pos+=1 end end
        local parse
        local function parseString()
            ...
            while pos <= #s do
                ...
                if c == '"' then pos+=1 return res end
                if c == '\\' then
                    pos+=1; c = s:sub(pos,pos)
                    if c=='n' then res..='\n' elseif c=='t' then res..='\t'
                    elseif c=='r' then res..='\r' else res..=c end
                else res..=c end
                pos+=1
            end
            return res
        ...
        local function parseNum()
            local s2, e = s:find("^-?%d+%.?%d*[eE]?[+-]?%d*", pos)
            local n = tonumber(s:sub(s2,e)); pos = e+1; return n
        end
        parse = function()
            skip()
            local c = s:sub(pos,pos)
            ...
            elseif c=='{' then
                ...
                while s:sub(pos,pos)~='}' do
                    local k = parseString(); skip()
                    pos+=1; skip() -- colon
                    t[k] = parse(); skip()
                    if s:sub(pos,pos)==',' then pos+=1 end; skip()
                end
                pos+=1; return t
            elseif c=='[' then
                pos+=1; local t={}; skip()
...
-- Save current state to a named config file
function Library:SaveConfig(Name: string)
    assert(typeof(Name)=="string" and #Name>0, "Config name must be a non-empty string.")
    local ok, err = EnsureConfigFolder(Library.ConfigPath)
    if not ok then
        warn("SaveConfig: " .. tostring(err)); return false
    end
    local encoded = JSONEncode(SerialiseState())
    local success, writeErr = pcall(writefile, ConfigFilePath(Name), encoded)
    if not success then
        warn("SaveConfig: failed to write — " .. tostring(writeErr)); return false
    end
    return true
end

-- Load a named config file and apply values
function Library:LoadConfig(Name: string)
    assert(typeof(Name)=="string" and #Name>0, "Config name must be a non-empty string.")
    if not isfile then warn("LoadConfig: isfile unavailable"); return false end
    local path = ConfigFilePath(Name)
    if not isfile(path) then
        warn("LoadConfig: config '" .. Name .. "' not found."); return false
    end
    local raw = readfile(path)
    local ok, data = pcall(JSONDecode, raw)
    if not ok or typeof(data) ~= "table" then
        warn("LoadConfig: failed to parse '" .. Name .. "'"); return false
    end
    ApplyState(data)
    return true
end
...
function Library:DeleteConfig(Name: string)
    assert(typeof(Name)=="string" and #Name>0, "Config name must be a non-empty string.")
    if not (isfile and delfile) then warn("DeleteConfig: filesystem unavailable"); return false end
    local path = ConfigFilePath(Name)
...
-- List all saved config names (without extension)
function Library:ListConfigs(): { string }
    if not (isfolder and listfiles) then return {} end
    if not isfolder(Library.ConfigPath) then return {} end
    local names = {}
    for _, file in listfiles(Library.ConfigPath) do
...
-- Enable/disable auto-save on every element value change
function Library:SetAutoSave(Enabled: boolean, Filename: string?)
    Library.AutoSave = Enabled
    Filename = Filename or "autosave"

    -- Clean up previous debounce instance
    if Library.AutoSaveConnection then
        pcall(function() Library.AutoSaveConnection:Destroy() end)
        Library.AutoSaveConnection = nil
    end

    if not Enabled then return end

    -- BindableEvent used as a debounced save trigger
    local Debounce = Instance.new("BindableEvent")
    local Pending = false

    Debounce.Event:Connect(function()
        if Pending then return end
        Pending = true
        task.delay(0.8, function()
            if Library.AutoSave then
                Library:SaveConfig(Filename)
            end
            Pending = false
        end)
    end)

    -- Store the BindableEvent so we can Destroy it later
    Library.AutoSaveConnection = Debounce

    -- Wrap all toggle callbacks to fire the debounce
    for _, Toggle in Library.Toggles do
        local OrigCB = Toggle.Callback
        Toggle.Callback = function(v)
            Library:SafeCallback(OrigCB, v)
            if not Debounce.Parent == nil then return end
            pcall(function() Debounce:Fire() end)
        end
    end

    -- Wrap all option callbacks to fire the debounce
    for _, Option in Library.Options do
        local OrigCB = Option.Callback
        Option.Callback = function(v)
            Library:SafeCallback(OrigCB, v)
            pcall(function() Debounce:Fire() end)
        end
    end
...
function Library:CreateWindow(WindowInfo)
    ...
    Library.ConfigPath = WindowInfo.ConfigPath
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddGroupbox(Info)
            ...
            function Groupbox:SetCollapsed(State: boolean)
                ...
                -- Show/hide content and divider
                DividerLine.Visible = not State
                GroupboxContainer.Visible = not State

                -- Resize holder directly (no tween needed — container hide avoids height race)
                if State then
                    GroupboxHolder.Size = UDim2.new(1, 0, 0, 34)
                else
                    Groupbox:Resize()
